...
| PS — Prairie Spray compliance presence | **RUNTIME-BLOCKED** | SR-007 records a local disposable manager desktop/phone packet from an uncommitted tree based on `b8ce4bd`: exact application/product snapshots, `100.00` to `92.50` gal arithmetic, browser-originated same-payload replay with one resulting application/product, named non-writes, phone Compliance details, no overflow, and no external browser network. Under this scorecard's immutable-evidence definition, it cannot become **PROVEN** until an authorized commit and fresh Sol exact-SHA review. The source's separate ID-bound Spray receipt hardening candidate remains unclaimed. |
...
| Playwright desktop + phone, PS | **RUNTIME-BLOCKED** | SR-007 records a passing disposable local desktop manager-create/replay lane and phone-sized Compliance read-only/no-overflow lane from an uncommitted tree based on `b8ce4bd`. It is not immutable accepted evidence and does not prove release/full-year readiness. |
