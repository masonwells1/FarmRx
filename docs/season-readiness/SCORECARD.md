...
|---|---|---|
| Custom SMTP and real password-email delivery | **AUTHORIZED / PENDING PROOF** | The provider is configured and an authenticated production Vercel environment check returned exact `VITE_PASSWORD_EMAIL_DELIVERY_ENABLED=true`. Disposable messages arrived before repair, but that stale-worker journey is not final release proof. Accepted source `2c4fadb1` is local only; the recovery hostname is NXDOMAIN/unbound and the exact Auth redirect is absent. Mason authorized the exact same-project domain/DNS, allowlist, disposable account/email, and external-Chrome proof sequence; none receives completion credit until observed. |
...
| Repository publication and public HTTP verification | **PROVEN FOR OLD MAIN; RECOVERY PENDING** | PR #16 is the current documentation-only tip of `main` at `45dc52f425ee844a0ddc473d514cce748e61c559`; current production is healthy at that exact SHA. Recovery head `2c4fadb1` remains local and receives no publication/live credit. |
