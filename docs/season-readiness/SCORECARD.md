...
**Snapshot:** accepted continuous Maple Ridge January-December runtime at `feb3ea7b0b4cc72cef61df45706916f1a83a41f6` (`feb3ea7`); accepted Prairie Spray runtime at `0753c5165116c6a8f25076dd4b036b9afb570d51` (`0753c51`)
...
| Accepted Maple exact-SHA verification ladder | **PROVEN** | SR-026/SR-027 record the full ladder, continuous January-December disposable runtime, step-exact mutation simulation, and fresh Sol acceptance at exact proof commit `feb3ea7b0b4cc72cef61df45706916f1a83a41f6`. This proves the Maple packet only; it is not an exact-current-HEAD release claim and does not prove the remaining scenario gauntlets or release readiness. |
| Full-year evidence packet and append-only closeout | **PROVEN** | SR-027 records the accepted one-reset continuous January-December Maple packet at `feb3ea7b0b4cc72cef61df45706916f1a83a41f6`. |
...
|---|---|---|---|
| January | **PROVEN** | Accepted proof HEAD `0238361192b7fa23d67956f43ffbf74be64c4022`; ledger SR-003/SR-004. | No additional Maple month proof is pending; release readiness still requires the other scenario gauntlets. |
| February | **PROVEN** | SR-005; accepted commit `53e8d2d`. Real local desktop UI created and assigned the exact Maple Program, followed by focused database assertions. | No additional Maple month proof is pending; release readiness still requires the other scenario gauntlets. |
| March | **PROVEN** | SR-005; accepted commit `53e8d2d`. Real local desktop UI received the exact Maple product, with target-write, identity, clock, network, and database assertions. | No additional Maple month proof is pending; release readiness still requires the other scenario gauntlets. |
| April | **PROVEN** | SR-005; accepted commit `53e8d2d`. Real local desktop UI inspected and cancelled without a write; the proof asserted zero target/unexpected writes and database invariance. | No additional Maple month proof is pending; release readiness still requires the other scenario gauntlets. |
| May | **PROVEN** | SR-005; accepted commit `53e8d2d`. Desktop marked the exact pass applied and phone-sized read-only UI confirmed it; database assertions passed. | No additional Maple month proof is pending; release readiness still requires the other scenario gauntlets. |
| June | **PROVEN** | SR-005 and SR-017/SR-018; accepted commit `021b587` reran January through June before entering July, preserving the accepted June SQL boundary. | No additional Maple month proof is pending; release readiness still requires the other scenario gauntlets. |
| July | **PROVEN** | SR-017/SR-018 and SR-027; exact commit `feb3ea7` reran the one-reset continuous chain through July before entering August. | No additional Maple month proof is pending; release readiness still requires the other scenario gauntlets. |
| August | **PROVEN** | SR-023 through SR-027; exact commit `feb3ea7` completed only the retained task, pinned server completion time/identity and `updated_at`, and passed desktop/phone/all-table non-write proof. | None for the Maple packet. |
| September | **PROVEN** | SR-023 through SR-027; exact commit `feb3ea7` saved the one exact harvest/replay receipt and passed desktop/phone/all-table non-write proof. | None for the Maple packet. |
| October | **PROVEN** | SR-023 through SR-027; exact commit `feb3ea7` proved the explicit two-write Grain estimate/reconciliation, complete row shape/timestamps, lost-response replay, and no hidden coupling. | None for the Maple packet. |
| November | **PROVEN** | SR-023 through SR-027; exact commit `feb3ea7` proved five independent writes with lost-response retries, exact values, and step-specific all-table baselines that retain every earlier row. | None for the Maple packet. |
| December | **PROVEN** | SR-023 through SR-027; exact commit `feb3ea7` ran twelve sequential read-only tests across all named modules, phone/no-overflow, and two fresh contexts observing both startup-status RPCs, with SQL and all-table invariance. | None for the Maple packet. |
...
| PH — Pine Hill offline/recovery | **RUNTIME-BLOCKED** | The bounded product repair now persists each new Field Log queue item's exact operation-era generation/token/server epoch, prevents legacy work from auto-sending while keeping it reviewable, and retrieves one authorized removed-farm epoch for authoritative revoked reset. Focused queue/UI/parser regressions and a disposable PostgreSQL security verifier cover the repaired boundaries. The governed Pine fixture/controller, exact desktop+phone sequence, browser byte assertions, and complete SQL/evidence packet remain absent. |
...
| Playwright desktop + phone, Maple August–December | **PROVEN** | SR-023 through SR-027 record the accepted one-reset continuous packet at exact commit `feb3ea7`, including source-real writes, phone/no-overflow lanes, exact database deltas, and read-only December reconciliation. This is browser emulation, not a physical-device journey. |
...
| Playwright desktop + phone, NF/CC/PH | **UNMAPPED/UNBUILT** | Required executable matrix and evidence packets are absent. Read-only mappings do not constitute executable browser coverage. |
