...
- Opening a role-visible direct module, a form, Weather, or a read-only report is not a write.
- Use the direct module navigation and source-real controls named below. The destination module owns validation, IDs, queues, submit locks, and receipts.
...
- Maple Ridge starts with its farm, owner membership, operating entity, and manifest Inventory product `Synthetic Herbicide 41 — Maple`; that product's canonical inventory unit is `gal`. It has no field, crop assignment, season activity, inventory ledger, Program, task, or application row before January. Grain setup contains only the manifest Maple cash-bid fixture: farm Maple Ridge, elevator `Synthetic Elevator`, commodity `corn_yellow`, `bid_date = 2027-11-10`, `basis = 0.000000`, `cash_price = 4.250000`, `delivery_start = 2027-11-10`, `delivery_end = 2027-12-15`, notes `Synthetic season fixture`. There is no Maple production-estimate, contract, contract-delivery, bin, bin-inventory, or bin-transaction row before the scenario creates it.
...
|---|---|---|---|
| Jan 12, `2027-01-12T08:00:00-06:00` | Open Fields and choose **Full field details** → **Add a field** (`/fields/new`). Save Maple East 160 under the manifest entity with 160.00 total ac and Jackson County. On the new field detail, use **Basics** → **Edit** to set state `IL`; **Land agreement** → **Edit** to retain owned and set **Terms effective from** to `2027-01-01`; **Records** → **Edit** → **Add crop record** to create `corn_yellow`, year `2027`, sequence `1`, and 160.00 planted ac; then **Yield & price** → **Edit** to set 200.0000 bu/ac expected yield. Planting/harvest/actual-price fields remain `null`; reload the field detail and Fields list. | Those Fields actions leave exactly the manifest field, owned arrangement, and crop-assignment rows with those values. | Merely opening Fields or a field-detail card performs no write. No separate setup-status row or standalone planting-actual row appears. |
| Feb 18, `2027-02-18T09:00:00-06:00` | Open Programs (`/programs`) and choose **New program**. Create `Maple 2027 Corn Program` (`program_kind = chemical`, `commodity_id = null`, `crop_year = 2027`). Add pass `Post-emerge synthetic pass` (`pass_type = post`, `activity_type = spray`, target `2027-05-20`, reminder lead `3` days) and product `Free-Typed Program Herbicide` (`rate_text = 10.00`, `unit_text = gal total`, estimated cost `$7.00/ac`). Use **Assign to fields** to assign it to the manifest Maple crop assignment, then reopen it through **My programs** and confirm it in **Season progress**. | Exactly one row each is created with the six manifest IDs in `programs`, `program_passes`, `program_pass_products`, `program_assignments`, `assigned_program_passes`, and `assigned_program_pass_products`. The Program row has `commodity_id = null`; crop identity comes from the explicit assignment to the manifest crop assignment. The assigned rows preserve the named snapshot values. | Inventory products/ledger, receipts, applications, tasks, notifications, Grain, and the crop assignment remain unchanged. No runtime-generated Program product UUID is accepted. |
| Mar 22, `2027-03-22T07:30:00-05:00` | Receive exactly 100.00 gal of `Synthetic Herbicide 41 — Maple` from `Synthetic Ag Supply`, received `2027-03-22`, using the manifest receipt and receipt-line IDs; see the confirmed receipt and 100.00 gal on hand. | Exactly one receipt and one receipt-line row are created. The receipt stores vendor `Synthetic Ag Supply` and purchase date `2027-03-22`; the line uses the manifest product, quantity 100.00, and entered/canonical inventory unit `gal`. On-hand changes from 0.00 to 100.00 gal. | No pending Crop RX delivery record or UI state, application, Program-product match, task, notification, or Grain row is created. |
| Apr 16, `2027-04-16T06:45:00-05:00` | Open Fields → Maple East 160 → **Records** → **Edit**. Verify the current existing-crop form exposes **Planted acres** and **Harvested bushels** but no planting-date input. Choose **Cancel** without submitting, then reload. | No write occurs. The manifest crop assignment remains byte-for-byte stable with `planting_date = null`; its row/version and every relevant table count remain unchanged. | No planting-event/planting-actual entity, machine-import row, or grain lot appears. This surfaces an existing-workflow hardening gap; this documentation tranche does not authorize adding a planting-date field or a standalone planting entity. |
...
| Jul 09, `2027-07-09T16:10:00-05:00` | Open Scouting (`/scouting`), find the Maple East 160 field card, and choose **New scouting note**. Save the manifest note with date `2027-07-09`, category `weed`, note `Synthetic waterhemp at south gate`, `latitude = null`, `longitude = null`, no photos, and **Add a follow-up task** unchecked. Then separately open Tasks (`/tasks`), choose **Add task**, and save the manifest task: title `Inspect Maple south gate`, details `Check synthetic waterhemp patch.`, `todo`, normal priority, assigned to owner user, due `2027-07-10`, linked to Maple East 160, source `manual`. Confirm both receipts. | Exactly one `scouting_notes` row and one separately created `farm_tasks` row contain those values. | Opening either direct module or form writes nothing. `scouting_photos` and `notifications` remain empty, no scouting-created task is generated, and no spray record or Program status changes. |
| Aug 17, `2027-08-17T13:00:00-05:00` | Open Tasks (`/tasks`), locate `Inspect Maple south gate` in **To Do**, choose **Done**, then reload. | Only the manifest task row changes: `status = done`, server-owned completion identity/time reflects the owner and simulated instant, and its version/`updated_at` advances once. | Opening Tasks does not create a task or mutate Programs, Inventory, Grain, Fields, scouting, or notifications. |
| Sep 28, `2027-09-28T18:05:00-05:00` | Open Harvest (`/harvest`), find Maple East 160, choose **Enter harvest**, enter 30,800.00 bu, date `2027-09-28`, and fictional actual price `$4.250000`, then choose **Save harvest**. Confirm the saved values and reload. | Only the existing Maple crop assignment's `harvested_bushels`, `harvest_date`, `actual_price_per_bu`, and version fields change. | Grain production remains projected; the Grain estimate, contracts, deliveries, and every bin row remain unchanged. |
| Oct 19, `2027-10-19T08:40:00-05:00` | Open Grain (`/grain`). In **Start your grain estimate**, enter **APH / expected yield** `200.0000` and choose **Create estimate** for the 2027 corn crop assignment. After the save reloads Grain into its position view, confirm Harvest actuals are 30,800.00 bu and Grain actual production is `not entered`. Choose **Use harvest total as Grain actual** and accept the exact confirmation `Use the harvest total as Grain actual? This changes Grain actual only; it does not change bins.` | Two ordered writes occur. First, exactly one `production_estimates` row is inserted with the manifest Maple production-estimate ID, Maple farm, `crop_year = 2027`, `commodity_id = corn_yellow`, `operating_entity_id = null`, and `enterprise_label = null`; the repository reconciles the existing crop assignment as 160.00 planted acres × 200.0000 = 32,000.00 expected bushels, so the row has `planted_acres = 160.00`, `aph_yield = 200.0000`, `expected_bushels = 32000.00`, `actual_bushels = null`, `drives_math = projected`, and `notes = null`. Second, that same row updates to `actual_bushels = 30800.00` and `drives_math = actual`; its APH, expected bushels, scope, and identity remain unchanged. | Across both actions, the crop assignment and Harvest actuals, the preseeded cash-bid fixture, and every bin, contract, delivery, lot, and unrelated row remain unchanged. No bin, lot, contract, or delivery is created automatically. |
| Nov 10, `2027-11-10T11:25:00-06:00` | Create manifest bin `Maple North Bin`, capacity 40,000.00 bu, on-farm, location `Maple Ridge`; add the manifest manual In movement for 30,800.00 bu `corn_yellow`, occurred `2027-09-28`, note `2027 harvest load-in`. In Contracts choose **Cash / spot** (`contract_type = cash_spot`) and the fixture-provided Buyer option `Synthetic Elevator`; create the manifest contract for 5,000.00 bu at `$4.25/bu`, delivery `2027-11-10` through `2027-12-15`, contract `MR-2027-001`, premium `0`, notes `null`. Separately add the manifest 5,000.00 bu manual Out movement dated `2027-11-10`, note `Delivery to Synthetic Elevator`. Separately record the manifest 5,000.00 bu contract delivery dated `2027-11-10`, note `null`. Capture local DB state after each button. | Five independent writes occur in order: one bin, one In transaction, one `cash_spot` contract, one Out transaction, one contract-delivery row. Bin on-hand is 30,800.00 after In and 25,800.00 after Out; delivered total is 0.00 until the delivery action, then 5,000.00. | The preseeded Maple cash-bid fixture remains unchanged. Bin creation/In does not create a contract or delivery. Contract creation does not move grain. Bin-out does not record delivery. Delivery does not add another bin movement. No `bin_inventory` baseline or grain lot auto-appears. |
| Dec 15, `2027-12-15T09:30:00-06:00` | Review the direct existing modules Fields, Grain, Inventory → **Compliance**, Programs, Harvest, Scouting, and Tasks. Reload each and confirm the year's totals remain coherent. | No write is expected from opening or reloading those modules. | No year-end finalization row/action, rollover mutation, Crop RX sync, vendor sync, or hidden cross-module reconciliation occurs. |
...
5. **CC-4 — Scouting while Weather is stale.** Open Scouting (`/scouting`), find the Cedar West 40 field card, choose **New scouting note**, and save the manifest note: date `2027-07-07`, category `weed`, note `Synthetic waterhemp along west edge`, `latitude = null`, `longitude = null`, `photos = []`, and `create_task = false`. Exactly one `scouting_notes` row is created and the receipt remains visible while the Weather lane is stale.
