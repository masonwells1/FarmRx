# Farm Rx Sol delta 11 release review
...
Farm Rx must keep every user and farm's data isolated. A read or write captures one authenticated user, farm, and server-owned access epoch and must not silently adopt a later session. If identity or access changes during an operation, later remote work, local queue/outbox/cache/receipt changes, retained data, and public success must stop. A same-user/same-farm read may retry only after a new access fence is captured; it must never retry an old invocation into a different user or farm. Server-owned scheduled work must remain farm-contained, and privileged functions must expose only their intended role surface.

The candidate claims all prior release blockers are closed, including these most recent five:
...
2. `record_marketing_alert_transition` calls the server-owned user/farm/access-epoch assertion before locking or changing its non-farm-keyed state table. A disposable two-user and stale-epoch drill proves rejection leaves state unchanged.
3. Push-subscription save/delete require `p_farm_id`, bind captured expected-user/epoch headers, assert them server-side before mutation, and retire the old unfenced function signatures.
4. `save_push_subscription` cannot transfer an existing endpoint to another user. Its atomic conditional upsert refreshes only the same owner and raises `PUSH_SUBSCRIPTION_OWNED_BY_ANOTHER_USER` otherwise. The disposable proof creates an old-owner pending delivery target, attempts the same endpoint under a second valid user/farm/epoch context, and proves ownership, keys, and the target stay unchanged while a same-owner refresh still works.
5. Every cached queued repository read captures a `FarmOperationContext` and verifies it after replay, after live reads, after cache I/O, and before public return. The production Scouting race regression pauses User A's live read, switches to User B on the same farm, resolves B data, and proves the old call rejects before IndexedDB is touched. Fields recovery retries only when user and farm are unchanged and the access fence advanced; the built-browser revoke/regrant proof covers that lane.
...
The orchestrator reports a fresh complete local gate passed after the latest fixes: forced TypeScript compilation; all 39 regression programs; production build; `npm audit --audit-level=high` with zero vulnerabilities; Deno check for `deliver-grain-alert`; static guards; 9/9 mutation drills; disposable migrations and probes through `0041`; the RLS role matrix; and 32/32 Chromium desktop/phone tests. Independently verify the most relevant evidence rather than merely repeating that claim. Label any command unavailable in the read-only environment as a verification limitation, not a defect.
...
3. A closure table for the five recent blockers and the four earlier repair groups, with exact production-code and deterministic-proof references.
