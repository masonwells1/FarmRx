...
1. **Wire inventory costs into budgets** (M). The database view `application_cost_lines` already produces per-application cost and $/acre by profitability category from real spray records; Profitability already renders `source_kind: 'inventory'` lines read-only. Build the importer (same pattern as the equipment importer, which is already done and well designed): total the eligible applications' cost and divide by the budget's allocated acres, never sum the per-application $/acre rows.
...
5. **Compliance that protects** (M). Compute REI re-entry time and PHI harvest-safe date per field and show them on Fields/Today, failing closed to "not on file, check the label" when a product's hours are missing; applicator roster so name and license are picked, not retyped; void/correct a completed spray record with a reason; compliance PDF for an inspector.
...
Not done here: running the app in a browser (there is no mock backend mode and no Docker in this environment), touching production, Vercel, or Supabase, or exercising any live service. Nothing was pushed during the audit itself; this document, the GOAL.md amendment, and two small CI repairs were pushed afterward on pull request #38 with Mason's approval. Competitor prices are from July 2026 research and should be re-checked before quoting them.
