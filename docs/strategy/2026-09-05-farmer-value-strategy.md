...
3. **The stickiest feature is inert.** Marketing alerts only evaluate when the owner opens the Grain page, and a price alert can only fire against a cash bid the farmer typed within the last 2 days. In practice a price target will never fire on its own. The scheduled server job that exists (`scheduled-alert-sweep`) is weather-only.
...
Order matters. Research in `docs/profitability-research-2026-07.md` is blunt: *"the weekly hook is marketing, not budgeting — budgets get opened twice a year."* So the first bet is the one that puts a live number in front of the farmer every day, and the second is the screen that shows it to him.
...
Add **Quick Record** as the same tranche: *Log rain · Record spray · Enter a load · Add task* from one button. Restore the two-tap rule the handoff and design principles require.
...
1. **Link Programs to the Inventory catalog** (M). `catalog_product_id` is already reserved on program products. In the program builder, pick a product from the shed (with "Add new" inline) and enter rate as a number + unit instead of free text. Keep free-typed as a fallback so nothing breaks.
2. **Then compute the plan** (M). Planned need = Σ rate × assigned acres per product. Show **planned vs on hand vs applied vs remaining** per product, and a *Short list* — what to buy. With the farmer's explicit consent, the short list is also the most natural Crop RX order in the world.
3. **Make "Mark applied" carry the products** (S). Today it creates an empty draft spray record. Prefill product, rate, and computed total from the pass so the compliance record is one confirmation away, and let a pass both create the record *and* draw down inventory (currently mutually exclusive).
