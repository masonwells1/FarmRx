...
**Owner:** Mason Wells · **Directed:** 2026-07-18 · **Current state:** RELEASE CANDIDATE READY; the six-scenario 2027 packet remains accepted at `8a9565a08a760e0ec920170bfacee1d9132cba47`, and the worker-free password-recovery hardening is locally accepted at `e012e718aec1f5492894891a3df92544c1c4951f`; current remote/production `main` remains `45dc52f425ee844a0ddc473d514cce748e61c559`; not COMPLETE
...
- The 2026-08-01 operator tranche configured the SMTP provider and guarded production email flag and received disposable recovery messages, but that pre-repair journey exposed the stale service-worker defect and does not count as final end-to-end proof. The accepted repair is not published, `recovery.croprxsolutions.app` is still unconfigured/NXDOMAIN, and the exact Supabase redirect is not yet enabled. Publication, automatic deployment, same-project domain/DNS binding, Auth allowlisting, and the final real-email proof remain separately approval-gated.
...
SR-041 adds the accepted local recovery hardening at exact immutable commit `e012e718aec1f5492894891a3df92544c1c4951f`. Its full foundation gate passed with 59 browser scenarios and 9 intentional configuration/project skips; focused default and enabled-email cross-origin packets passed; the season contract remained green; 17/17 controlled foundation mutations were rejected; and fresh-context read-only Sol returned **ACCEPT** with no merge blocker. The repair gives recovery a dedicated worker-free origin, confines that hostname to `/update-password`, returns farmers to the canonical app, preserves ordinary sessions on cancel/retry, and clears a prior canonical session only after confirmed password completion. Provisioning and the operational runbook are pinned to the exact recovery URL and safe deployment sequence.

The initiative remains **RELEASE CANDIDATE READY**, not **COMPLETE**. The accepted recovery SHA has not been pushed, placed in a PR, merged, or deployed. The recovery hostname/DNS and exact Supabase redirect are not configured, so no final no-bypass live Chrome/email proof exists. Physical iPhone/Safari and Android/Chrome installed-PWA journeys also remain unperformed. Weather-to-spray remains manual payload-free transcription with no provenance link, integration, or automatic prefill. No migration, live data, permission, customer-account, or customer-communication action occurred in SR-041.
