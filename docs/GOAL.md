...
- The 2026-08-01 operator tranche configured the SMTP provider, and an authenticated Vercel check confirmed exact production `VITE_PASSWORD_EMAIL_DELIVERY_ENABLED=true`; disposable recovery messages arrived, but that pre-repair journey exposed the stale service-worker defect and does not count as final end-to-end proof. The controlling PR-review repair is freshly accepted at exact `9ecc6cba7bbaa9bea063054fe3b996cd22ea9555`: it retains authenticated completion cleanup and now fails honestly before the provider call when cleanup-marker storage is unsafe, while the runbook stops unless the no-real-farmer customer-zero boundary and retirement of every known proof client are proven. That SHA is local only; PR #17 still points to `54775ea67e92122c99fd677249cc1cef8d7cea32`, production/main remains `45dc52f425ee844a0ddc473d514cce748e61c559`, `recovery.croprxsolutions.app` is unconfigured/NXDOMAIN, and the exact Supabase redirect is not enabled. Mason has authorized this exact publication/deployment/domain/Auth/disposable-email sequence, but none of those pending actions receives completion credit yet.
...
SR-041/SR-042 preserve predecessor recovery acceptances through exact `2c4fadb1d8f4bbffe025dc92aa1a79caa02efba4`. SR-044 records the controlling accepted source at exact `9ecc6cba7bbaa9bea063054fe3b996cd22ea9555`: its complete foundation gate passed with 62 browser scenarios and 14 intentional skips, 25/25 controlled mutations were rejected, focused enabled-email recovery passed 11/11, the season contract remained green, and fresh-context read-only Sol returned **ACCEPT — no merge blockers**. The repair retains authenticated completion cleanup, adds an honest visible storage-preflight failure before any provider request, and makes stale-client retirement a mandatory customer-zero transition gate.

SR-042 records Mason's explicit authorization for this exact publication, automatic deployment, recovery-domain/DNS, Supabase redirect, disposable Auth/email/external-Chrome proof and cleanup, and follow-up governed closeout sequence; SR-044 does not expand it. The initiative remains **RELEASE CANDIDATE READY**, not **COMPLETE**: accepted `9ecc6cb` is still local while PR #17 points to older `54775ea`; no accepted recovery source is merged or deployed; customer-zero stale-client retirement is not yet evidenced; the recovery hostname/DNS and exact Supabase redirect are not configured; and no final no-bypass live Chrome/email proof exists. Physical iPhone/Safari and Android/Chrome installed-PWA journeys also remain unperformed. Weather-to-spray remains manual payload-free transcription with no provenance link, integration, or automatic prefill. No migration, customer data, real farmer account, or customer communication is authorized or credited.
