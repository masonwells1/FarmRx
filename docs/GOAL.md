...
SR-045 records the performed outward sequence. SR-046 corrects its provenance boundary without rewriting it: repository, deployment, domain/DNS/TLS, redirect configuration, redacted Auth event chain, exact deletion, and retained-user count are durable; browser/mailbox-only statements are **OPERATOR-OBSERVED**. The redacted evidence index is the controlling map for that distinction. No migration, customer data, real farmer account, permanent farm access, or customer communication was performed.
