...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (kind.startsWith('marketing_')) { const ruleId = clean(body.ruleId, 36); if (!uuid(ruleId)) return json({ error: 'Invalid alert.' }, 400); const { data: rule, error: ruleError } = await admin.from('marketing_alert_rules').select('id,rule_type,active,crop_year,commodity_id,threshold,direction,remind_on').eq('id', ruleId).eq('farm_id', farmId).maybeSingle(); if (ruleError) throw ruleError; const expectedType = kind === 'marketing_price_target' ? 'price_target' : kind === 'marketing_pct_marketed_goal' ? 'pct_marketed_goal' : 'deadline'; if (!rule || !rule.active || rule.rule_type !== expectedType || (expectedType === 'price_target' && (typeof rule.threshold !== 'number' || !['at_or_above', 'at_or_below'].includes(rule.direction ?? ''))) || (expectedType === 'pct_marketed_goal' && typeof rule.threshold !== 'number') || (expectedType === 'deadline' && !rule.remind_on)) return json({ error: 'Alert is no longer current.' }, 409); const commodity = clean(rule.commodity_id, 80); if (kind === 'marketing_price_target') { subject = 'Farm Rx price target reached'; message = `${rule.crop_year} ${commodity} reached your saved cash-price target.` } else if (kind === 'marketing_pct_marketed_goal') { subject = 'Farm Rx marketed goal reminder'; message = `${rule.crop_year} ${commodity} is below your saved marketed goal.` } else { subject = 'Farm Rx marketing deadline'; message = `${rule.crop_year} ${commodity} has a saved reminder date of ${clean(rule.remind_on, 10)}.` } }
