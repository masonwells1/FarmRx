...
function New-Mock([string]$FailAction='',[string]$FailWhen=''){
  ...
  $a.StopOriginal=New-Mutation stop ({$state.OriginalRunning=$false;$state.OriginalHealthy=$false}.GetNewClosure()) $log $FailAction $FailWhen
  ...
  $a.ProveRouteClockAndLineage={$true}
  ...
  $a.StartOriginal={$log.Add('start-original');$state.OriginalRunning=$true;$state.OriginalHealthy=$true;$true}.GetNewClosure()
