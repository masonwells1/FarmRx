...
$parameterized=New-Mock;$parameterizedAction={param([string]$Name,[string]$ClientInstant,[bool]$ReadOnly,[string]$WriteTag,[string]$PhoneTag,[string]$SqlPath);Assert-True($Name-ceq'august'-and$ClientInstant-ceq'2027-08-17T13:00:00-05:00'-and-not$ReadOnly-and$WriteTag-ceq'@maple-august-write'-and$PhoneTag-ceq'@maple-august-phone'-and$SqlPath-ceq'maple.verify.sql')'frozen action parameters were not passed explicitly';$true};$parameterizedArgs=@{Name='august';ClientInstant='2027-08-17T13:00:00-05:00';ReadOnly=$false;WriteTag='@maple-august-write';PhoneTag='@maple-august-phone';SqlPath='maple.verify.sql'};Assert-True((Invoke-MapleSwapStateMachine $parameterized.Adapter $inventory $parameterizedAction $parameterizedArgs)-ceq'MAPLE_DB_CLOCK_SWAP_ADAPTER_PASS')'explicit frozen action parameter contract failed'
...
$winter=$inventory.Clone();$winter.derived_tag='farmrx-frozen-clock-swap:20270209-ba10e934';Assert-True((Assert-MapleSwapInventory $winter $winter)-eq$true)'exact winter 2027 inventory was refused'
$extra=$inventory.Clone();$extra.extra='x'
foreach($bad in @($extra,@{contract_hash='bad';network_id=('b'*64);original_id=('c'*64);original_image_id=('sha256:'+('d'*64));snapshot_tag=('farmrx-clock-snapshot:'+('e'*12));derived_tag='farmrx-frozen-clock-swap:20270200-ba10e934';volume_name='supabase_db_farmrx-farmer-simplicity-2027-local'})){try{Assert-MapleSwapInventory $bad $inventory|Out-Null;$accepted=$true}catch{$accepted=$false};Assert-True(-not$accepted)'invalid inventory accepted'}
