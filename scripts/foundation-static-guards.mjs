...
export function foundationStaticGuard(root = process.cwd()) {
  ...
  if (/\bcatch\b|\.list\(/.test(soilStorageRemove)) errors.push('soil-rx:storage-remove-ambiguous-recovery-refused')
  ...
  for (const [required, label] of [["create function public.verify_soil_test_absent(p_farm_id uuid, p_test_id uuid)", 'row-function'], ["create function public.verify_soil_report_objects_absent(p_farm_id uuid, p_paths text[])", 'storage-function'], ['perform public.assert_current_farm_access_epoch(p_farm_id);', 'epoch'], ['not public.can_edit_farm(p_farm_id)', 'edit-access'], ["object.bucket_id = 'soil-test-reports' and object.name = requested.path", 'physical-object'], ['test.farm_id = p_farm_id and test.id = p_test_id', 'physical-row'], ['grant execute on function public.verify_soil_test_absent(uuid, uuid)', 'authenticated-grant']]) requireText(errors, soilMigration, required, `soil-rx:absence-rpc-${label}`)
