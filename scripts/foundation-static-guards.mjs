...
export function foundationStaticGuard(root = process.cwd()) {
  ...
  if ((pushAccessRevocation.match(/set search_path = public, pg_temp/g) ?? []).length !== 3) errors.push('push:security-definer-fixed-search-paths')
  ...
  requireText(errors, pushAccessRevocation, "last_error = 'farm access removed'", 'push:send-time-revalidation-terminal-reason')
