...
$proof = @{
    ArtifactImageRef='maple-faketime-artifacts-b9ad08aeb66ed961e8426b2cce527365@sha256:7cbc0a183ba33c4318a9784dae376104e55282e8e0c716511336afaf924f3302'; ArtifactImageId='sha256:7cbc0a183ba33c4318a9784dae376104e55282e8e0c716511336afaf924f3302'
...
function New-Simulator {
    ...
    $state = @{
        ...
        ArtifactMissing=$false;ArtifactInspectFailure=$false;ArtifactMalformed=$false;ArtifactId='sha256:7cbc0a183ba33c4318a9784dae376104e55282e8e0c716511336afaf924f3302'
        ArtifactLabels=@{'farmrx.synthetic-bootstrap'='b9ad08aeb66ed961e8426b2cce527365';'farmrx.synthetic-owner'='maple-faketime-bootstrap';'farmrx.synthetic-role'='faketime-artifacts';'farmrx.source-digest'='debian@sha256:7b140f374b289a7c2befc338f42ebe6441b7ea838a042bbd5acbfca6ec875818';'farmrx.package-contract'='libfaketime=0.9.10-2.1;gcc;libc6-dev'}
        ArtifactTagMissing=$false;ArtifactTagInspectFailure=$false;ArtifactTagMalformed=$false;ArtifactTagId='sha256:7cbc0a183ba33c4318a9784dae376104e55282e8e0c716511336afaf924f3302'
        ArtifactTagLabels=@{'farmrx.synthetic-bootstrap'='b9ad08aeb66ed961e8426b2cce527365';'farmrx.synthetic-owner'='maple-faketime-bootstrap';'farmrx.synthetic-role'='faketime-artifacts';'farmrx.source-digest'='debian@sha256:7b140f374b289a7c2befc338f42ebe6441b7ea838a042bbd5acbfca6ec875818';'farmrx.package-contract'='libfaketime=0.9.10-2.1;gcc;libc6-dev'}
    }
...
try {
    ...
    $expectedBuild=@('build','--no-cache','--network=none','--pull=false','--label',"farmrx.maple-clock-swap=$owner",'--label','farmrx.maple-clock-role=frozen-derived','--label',"farmrx.maple-original-id=$originalId",'--label',("farmrx.maple-contract-hash="+('a'*64)),'--label',"farmrx.maple-clock-snapshot-id=$($sim.State.SnapshotId)",'--build-arg',"BASE_IMAGE=$($inventory.snapshot_tag)",'--build-arg','FAKETIME_ARTIFACTS_IMAGE=maple-faketime-artifacts-b9ad08aeb66ed961e8426b2cce527365:synthetic','--build-arg','FROZEN_INSTANT=2027-07-09 21:10:00+00:00','-f','tests/season/frozen-postgres-clock-spike.Dockerfile','-t',$inventory.derived_tag,'.')
    ...
    $imageRemovals=@($sim.Calls|Where-Object{$_.Argv.Count-ge3-and$_.Argv[0]-ceq'image'-and$_.Argv[1]-ceq'rm'}|ForEach-Object{$_.Argv[2]})
    Assert-True ($imageRemovals.Count-eq2-and@($imageRemovals|Where-Object{$_-cne$inventory.derived_tag-and$_-cne$inventory.snapshot_tag}).Count-eq0) 'cleanup must remove only the exact owned derived and snapshot images.'
    Assert-True (@($sim.Calls|Where-Object{($_.Argv-join' ')-match'(?i)\b(?:image|system)\s+prune\b'}).Count-eq0) 'cleanup must not issue broad image or system prune.'
    foreach($expected in @(
    ...
    foreach($expected in @(
        ...
        @('image','inspect','--format',$imageFormat,$inventory.snapshot_tag),@('image','inspect','--format',$imageFormat,$inventory.derived_tag),@('image','inspect','--format',$imageFormat,$proof.ArtifactImageRef),@('image','inspect','--format',$imageFormat,'maple-faketime-artifacts-b9ad08aeb66ed961e8426b2cce527365:synthetic'),
    ...
    $exactArtifactLabels=@{'farmrx.synthetic-bootstrap'='b9ad08aeb66ed961e8426b2cce527365';'farmrx.synthetic-owner'='maple-faketime-bootstrap';'farmrx.synthetic-role'='faketime-artifacts';'farmrx.source-digest'='debian@sha256:7b140f374b289a7c2befc338f42ebe6441b7ea838a042bbd5acbfca6ec875818';'farmrx.package-contract'='libfaketime=0.9.10-2.1;gcc;libc6-dev'}
