...
function New-MapleFaketimeArtifactBootstrapPlan([guid]$Id){
  ...
  [pscustomobject]@{
    ...
    FrozenOfflineBuild=[ordered]@{Resolved=$false;RequiresArtifactImageId=$true;ArgvTemplate=@('build','--network=none','--pull=false','--build-arg','BASE_IMAGE=<verified-local-digest-ref>','--build-arg','FAKETIME_ARTIFACTS_IMAGE=<inspected-sha256-image-id>','-f','tests/season/frozen-postgres-clock-spike.Dockerfile','-t','<synthetic-derived-tag>','.')}
...
function Assert-MapleFaketimeArtifactBootstrapPlan($Plan){
  ...
  $evidenceContract=[ordered]@{Resolved=$false;Required=@('exact image ID','all exact labels','source digest','package contract');RecordOnlyNonsecret=$true}
  $offline=[ordered]@{Resolved=$false;RequiresArtifactImageId=$true;ArgvTemplate=@('build','--network=none','--pull=false','--build-arg','BASE_IMAGE=<verified-local-digest-ref>','--build-arg','FAKETIME_ARTIFACTS_IMAGE=<inspected-sha256-image-id>','-f','tests/season/frozen-postgres-clock-spike.Dockerfile','-t','<synthetic-derived-tag>','.')}
  $cleanup=[ordered]@{InspectExactTagFirst=$true;RequireExactRecordedImageId=$true;RequireExactOwnershipLabel=$owner;RemoveArgv=@('image','rm',$tag);Force=$false;OnMissingOrAmbiguous='retain evidence and refuse'}
