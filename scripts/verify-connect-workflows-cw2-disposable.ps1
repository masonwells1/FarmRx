...
function Assert-Cw2Contract {
  ...
  if($derivedProof.contract-cne'The derived image was built locally with --pull=false from the reviewed reusable artifact tag after both tag and immutable ref were verified to the same ID and labels. The proof image had zero container references, the ordinary DB remained pinned and healthy, and only the exact proof image was removed.'){throw 'CONNECT_WORKFLOWS_CW2_DERIVED_IMAGE_PROOF_CUSTODY_DRIFT'}
  ...
  if($postcleanupProof.contract-cne'After exact owned derived-image removal, the reusable artifact immutable ref and mutable tag were re-inspected to the reviewed ID and five labels; it had zero container references and the ordinary DB remained unchanged and ready.'){throw 'CONNECT_WORKFLOWS_CW2_REUSABLE_POSTCLEANUP_CUSTODY_DRIFT'}
