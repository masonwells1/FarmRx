...
$migration = '20260813133808_connect_workflows_program_inventory.sql'
...
function Test-Cw2ArtifactManifestFocusedMatrix([string]$Source) {
  try {
    ...
    return (Get-Cw2Proof005TextSha256 $span)-ceq'c7a29c368c18e24269a3f64afa75ce36bae3ad1992beca79864695ead310ce55'-and(Get-Cw2Proof005TextSha256 $childSpan)-ceq'01551b9dad86fafa82b99224c2f2317d699be3a08edd1863cd9467a7b9dae092'
  } catch { return $false }
}
...
function Assert-Cw2Contract {
  ...
  if ($harvestClockSource -notmatch [regex]::Escape($replacementArtifactRef) -or $clockAdapterSource -notmatch [regex]::Escape($replacementArtifactRef) -or $clockAdapterRegressionSource -notmatch [regex]::Escape($replacementArtifactRef) -or $topologyPlanSource -notmatch [regex]::Escape($replacementArtifactRef) -or $topologyPlanRegressionSource -notmatch [regex]::Escape($replacementArtifactRef) -or $clockSpikeRunnerSource -notmatch [regex]::Escape($replacementArtifactTag) -or $clockSpikeRunnerSource -notmatch [regex]::Escape($replacementArtifactRef) -or $clockSpikeRunnerSource -notmatch [regex]::Escape($replacementArtifactId) -or $clockSpikeRunnerSource -notmatch [regex]::Escape('function Assert-ExactReusableArtifact') -or $clockSpikeRunnerSource -notmatch [regex]::Escape('"FAKETIME_ARTIFACTS_IMAGE=$artifactRef"')) { throw 'CONNECT_WORKFLOWS_CW2_REPLACEMENT_FAKETIME_ARTIFACT_REF_MISSING' }
  ...
  $historicalArtifactEvidence=$artifactEvidenceManifest.evidence
  if($historicalArtifactEvidence.custody-cne'historical-untracked-record-only; raw files are not an executable input. Current proof validates these tracked build-input pins and independently inspects the reusable artifact.'){throw 'CONNECT_WORKFLOWS_CW2_FAKETIME_ARTIFACT_TRACKED_CUSTODY_DRIFT'}
  $trackedBuildInputs=$artifactEvidenceManifest.build_inputs
  if($trackedBuildInputs.dockerfile.path-cne'tests/season/faketime-artifacts.Dockerfile'-or$trackedBuildInputs.dockerfile.sha256-cne'26eded17061445add0e3c4b43ad0c8dd8552d6745d6d615969372f9dfbc696a3'-or$trackedBuildInputs.copied_local_inputs.'tests/season/clear-ld-preload.c'-cne'b6d9b439ccbfdf88f87b9c2f2d89b560d2370964074759373949c2bbb67cd66e'){throw 'CONNECT_WORKFLOWS_CW2_FAKETIME_ARTIFACT_TRACKED_BUILD_INPUT_DRIFT'}
  $dockerfileInputHash = Get-Cw2Sha256 ([IO.File]::ReadAllBytes((Join-Path $root $trackedBuildInputs.dockerfile.path)))
  $preloadInputHash = Get-Cw2Sha256 ([IO.File]::ReadAllBytes((Join-Path $root 'tests/season/clear-ld-preload.c')))
  if ($dockerfileInputHash -cne $trackedBuildInputs.dockerfile.sha256 -or $preloadInputHash -cne $trackedBuildInputs.copied_local_inputs.'tests/season/clear-ld-preload.c') { throw 'CONNECT_WORKFLOWS_CW2_FAKETIME_ARTIFACT_TRACKED_BUILD_INPUT_SOURCE_HASH_DRIFT' }
  ...
  if($derivedProof.contract-cne'The derived image was built locally with --pull=false from the reviewed reusable artifact tag after both tag and immutable ref were verified to the same ID and labels. The proof image had zero container references, the ordinary DB remained pinned and healthy, and only the exact proof image was removed.'-or$derivedProof.custody-cne'historical-untracked-record-only; raw files are not an executable input. Current proof validates the tracked artifact identity and performs a fresh reusable-artifact inspection.'){throw 'CONNECT_WORKFLOWS_CW2_DERIVED_IMAGE_PROOF_CUSTODY_DRIFT'}
  ...
  if($postcleanupProof.contract-cne'After exact owned derived-image removal, the reusable artifact immutable ref and mutable tag were re-inspected to the reviewed ID and five labels; it had zero container references and the ordinary DB remained unchanged and ready.'-or$postcleanupProof.custody-cne'historical-untracked-record-only; raw files are not an executable input. Current proof validates the tracked artifact identity and performs a fresh reusable-artifact inspection.'){throw 'CONNECT_WORKFLOWS_CW2_REUSABLE_POSTCLEANUP_CUSTODY_DRIFT'}
  ...
  $artifactPreviousCommitFallback=$artifactManifestRegressionSource.IndexOf("Invoke-Cw2ArtifactGitPathList @('diff-tree','--no-commit-id','--name-status','-r','-z','-M100%','HEAD^','HEAD') 'FAKETIME_ARTIFACT_MANIFEST_PREVIOUS_COMMIT_DIFF_GIT_FAILED'")
...
'@
  ...
  if ($cw1Source -notmatch [regex]::Escape("`$migrationHead = '$fkIndexMigration'") -or $cw1Source -notmatch [regex]::Escape('if ($head -cne $migrationHead)')) { throw 'CONNECT_WORKFLOWS_CW2_CW1_MIGRATION_HEAD_GUARD_CHANGED' }
