...
try {
  ...
  reset()
  mutate('supabase/migrations/20260921180000_ld4_bin_origin_lot.sql', (source) => source.replace("  perform 1 from public.bin_transactions\n   where grain_load_id = p_load_id and farm_id = p_farm_id\n     and not (grain_bin_id = any(v_locked_bins));\n  if found then\n    raise exception 'this ticket was still being saved when the void started, so nothing was changed';\n  end if;\n", ''))
  detected('the void never checks that the bins it locked cover the load it found, so a save committing between the two reads deadlocks it', 'ld4:the-void-confirms-the-bins-it-locked')
  reset()
  mutate('supabase/migrations/20260921180000_ld4_bin_origin_lot.sql', (source) => source.replace('  perform public.lock_farm_bins(p_farm_id, v_locked_bins);', '  perform public.lock_farm_bins(p_farm_id, array(\n    select distinct grain_bin_id from public.bin_transactions\n     where grain_load_id = p_load_id and farm_id = p_farm_id));'))
  detected('the void locks one read of the bins and confirms a different one, so the set it checked is not the set it holds', 'ld4:the-void-confirms-the-bins-it-locked')
  reset()
  mutate('supabase/migrations/20260921180000_ld4_bin_origin_lot.sql', (source) => source.replace("  v_locked_bins := array(\n    select distinct grain_bin_id from public.bin_transactions\n     where grain_load_id = p_load_id and farm_id = p_farm_id);\n  perform public.lock_farm_bins(p_farm_id, v_locked_bins);\n\n  select * into v_load from public.grain_loads\n    where id = p_load_id and farm_id = p_farm_id for update;\n  if not found then raise exception 'that load does not belong to this farm'; end if;", "  select * into v_load from public.grain_loads\n    where id = p_load_id and farm_id = p_farm_id for update;\n\n  v_locked_bins := array(\n    select distinct grain_bin_id from public.bin_transactions\n     where grain_load_id = p_load_id and farm_id = p_farm_id);\n  perform public.lock_farm_bins(p_farm_id, v_locked_bins);\n  if not found then raise exception 'that load does not belong to this farm'; end if;"))
