param([Parameter(Mandatory)][string]$FrozenInstant,[switch]$Execute,[switch]$UnsafeReviewedAdapter)
...
$db='supabase_db_farmrx-farmer-simplicity-2027-local'; $digest='public.ecr.aws/supabase/postgres@sha256:ba10e934f0a59990379f78ab9ed93926f1c291dd61a12fe4026f4202f1b89770'; $journal=Join-Path $env:TEMP 'farmrx-maple-db-clock-swap-journal.json'; $envFile=Join-Path $env:TEMP 'farmrx-maple-db-clock-swap.env'
...
if (-not $UnsafeReviewedAdapter) {
  throw 'MAPLE_DB_SWAP_REFUSED: -Execute also requires the separately reviewed -UnsafeReviewedAdapter gate.'
}
# The adapter contract and recovery state machine are reviewed locally, but no
# concrete Docker adapter is integrated or invoked by this runner. A future adapter must create and clean an opaque,
...
throw 'MAPLE_DB_SWAP_REFUSED: concrete Docker adapter is not integrated or invoked; use the locked contract/state-machine proof only.'
