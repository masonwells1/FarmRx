...
Assert-True ($dockerfile.Contains('debian@sha256:7b140f374b289a7c2befc338f42ebe6441b7ea838a042bbd5acbfca6ec875818') -and $dockerfile.Contains('FROM ${BASE_IMAGE}') -and $dockerfile.Contains('libfaketime=0.9.10-2.1') -and $dockerfile.Contains('/usr/local/lib/faketime/libfaketime.so.1') -and $dockerfile.Contains('/usr/local/lib/faketime/libclear-ld-preload.so') -and $dockerfile.Contains('ENTRYPOINT ["/usr/local/bin/frozen-postgres-entrypoint"]') -and $dockerfile.Contains('CMD ["postgres", "-D", "/etc/postgresql"]') -and -not $dockerfile.Contains('LD_PRELOAD=')) 'Frozen-image feasibility contract changed.'
