...
import type {
  BudgetCostLine,
  BudgetFieldAllocation,
  CostCategory,
  CropBudget,
  EquipmentCostSnapshotPreview,
  EquipmentCostSnapshotRequest,
  ProfitabilityEquipment,
  ProfitabilityMatrixStep,
...
import {
  extraBushelsToJustify,
  missingCoachCategories,
  planCushions,
  planGroup,
  ...
  roiVerdict,
  roiWhatIfPerAcre,
} from "./data/planningTools";
import {
  FARMDOC_2026,
  FARMDOC_SOURCE_NOTE,
  farmdocCropKind,
  farmdocTypicalLine,
  type FarmdocCropKind,
...
const categories: Array<{ value: CostCategory; label: string }> = [
  ...
  { value: "equipment_depreciation", label: "Equipment" },
...
const arrangementLabels = {
  owned: "Owned",
  cash_rent: "Cash rent",
  flex_cash_rent: "Flex cash rent",
  crop_share: "Crop share",
} as const;
const PROFITABILITY_TABS = [
  { slug: "", label: "Overview" },
  { slug: "budgets", label: "Budgets" },
  { slug: "plans", label: "Compare plans" },
...
const sortedSteps = (
  ...
  workspace.matrix_steps
    .filter((step) => step.budget_id === budgetId && step.axis === axis)
    .slice()
...
function readCoachDismissed(): string[] {
  try {
    return JSON.parse(
      window.localStorage.getItem(COACH_KEY) ?? "[]",
    ) as string[];
  } catch {
    return [];
  }
...
function universityBudget(
  ...
  const budget: CropBudget = {
    ...
    name: `${cropYear} ${commodity.name} — U of I start`,
    expected_yield_per_acre: preset.expected_yield_per_acre,
    expected_price_per_bushel: preset.expected_price_per_bushel,
    rp_coverage_pct: null,
    rp_aph_yield: null,
    rp_projected_price: null,
    rp_premium_per_acre: null,
    copied_from_budget_id: null,
    created_at: at,
    updated_at: at,
  };
  const lines: BudgetCostLine[] = preset.lines.map((line) => ({
    id: crypto.randomUUID(),
    budget_id: budget.id,
...
function matrixColor(value: number, max: number) {
  ...
  if (value < 0)
    return `rgb(${198 + Math.round(45 * (1 - intensity))} ${40 + Math.round(75 * (1 - intensity))} ${40 + Math.round(45 * (1 - intensity))})`;
  if (value > 0)
    return `rgb(${40 + Math.round(130 * (1 - intensity))} ${128 + Math.round(34 * (1 - intensity))} ${88 + Math.round(64 * (1 - intensity))})`;
  return "var(--warn-bg)";
}

function stepsFromRange(
  budgetId: string,
  axis: "price" | "yield",
  min: number,
  max: number,
  ...
  if (
    ...
    throw new Error("Enter a sensible low, high, and step for the matrix.");
  const values: number[] = [];
  for (
    let value = min;
    value <= max + step / 1000 && values.length < 10;
    value += step
  )
    values.push(Number(value.toFixed(4)));
  if (values.length < 2)
    throw new Error("Use at least two steps on each matrix axis.");
  return values.map((value, sort_order) => ({
    id: crypto.randomUUID(),
    budget_id: budgetId,
    axis,
    value,
...
export function ProfitabilityPage() {
  ...
  const [applicationRecordsAvailable, setApplicationRecordsAvailable] =
    useState(true);
  ...
  const [collapsedCategories, setCollapsedCategories] = useState<
    Set<CostCategory>
  >(() => new Set());
  const writeLocks = useRef(createSubmitLockMap());
  const allocationLock = useRef(createSubmitLock());
  const navigate = useNavigate();
  const rawTab = useLocation().pathname.split("/")[2] ?? "";
  const tabPath = ["budgets", "plans", "reports"].includes(rawTab)
    ? rawTab
    : "";
  const refresh = async () => {
    try {
      ...
      setError(
        caught instanceof Error &&
          caught.message.includes("private on this farm")
          ? caught.message
          : farmerError(caught, "open profitability"),
      );
    }
  };
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (budget)
      setPickedCell({
        ...
        yield: budget.expected_yield_per_acre,
      });
  }, [selectedId]);
  useEffect(() => {
    const years = [
    ...
    setOverviewYear((current) =>
      current !== null && years.includes(current)
        ? current
        : (years[0] ?? null),
    );
  }, [workspace]);
  ...
  const saveAllocation = async (allocation: BudgetFieldAllocation) => {
    if (!allocationLock.current.acquire()) return;
    setSavingAllocation(true);
    try {
      await save(allocation.budget_id, () => profitabilityRepository.saveAllocation(allocation));
    } finally {
      allocationLock.current.release();
      setSavingAllocation(false);
    }
  };
  if (!workspace)
    return (
      <section className="page profitability-page">
        <h1>Profitability</h1>
        <p role="status">{error || "Opening your budgets…"}</p>
      </section>
    );
  const budget = workspace.budgets.find((item) => item.id === selectedId);
  if (!budget) {
      ...
      <section className="page profitability-page">
        ...
        <button
          ...
          onClick={() => {
            ...
            const next: CropBudget = {
              ...
              operating_entity_id: null,
              enterprise_label: null,
              name: `${new Date().getFullYear()} ${commodity.name}`,
              expected_yield_per_acre: 180,
              expected_price_per_bushel: 4.5,
              rp_coverage_pct: null,
              rp_aph_yield: null,
              rp_projected_price: null,
              rp_premium_per_acre: null,
            ...
            void save(next.id, () => profitabilityRepository.createBudget(next)).then(
              (saved) => saved && setSelectedId(next.id),
            );
          }}
        >
          Create first budget
        ...
        {(() => {
          const starter = workspace.fields.commodities
            .map((item) => ({
              item,
              kind: farmdocCropKind(item.crop_family ?? item.name),
            }))
          ...
          return (
            <button
              ...
              onClick={() => {
                ...
                }).then((saved) => saved && setSelectedId(created.budget.id));
              }}
            >
              Start from the 2026 U of I budget
            </button>
          );
        })()}
      </section>
  ...
  const selected = pickedCell ?? {
    price: budget.expected_price_per_bushel,
    yield: budget.expected_yield_per_acre,
  };
  const selectedProfit = matrixProfitPerAcre(
    selected.price,
    selected.yield,
    costsPerAcre,
  );
  const selectedAcres = workspace.allocations
    .filter((item) => item.budget_id === budget.id)
    .reduce((sum, item) => sum + item.allocated_acres, 0);
  ...
  const overviewBudgets = workspace.budgets.filter(
    ...
    <section className="page profitability-page">
      ...
      {landlordReportOpen && (
        <LandlordReport
          ...
          programs={programs}
          applicationRecordsAvailable={applicationRecordsAvailable}
          onClose={() => setLandlordReportOpen(false)}
        />
      )}
      {error && (
        <p className="profitability-error" role="alert">
      ...
      {tabPath === "budgets" && (
        <>
          <BudgetControls
            ...
            onCreate={(next) =>
              save(next.id, () => profitabilityRepository.createBudget(next)).then((saved) =>
                saved && setSelectedId(next.id),
              )
            }
            onCopy={(sourceId, copy) =>
              save(copy.id, () =>
            ...
            onCreateUniversity={(kind) =>
              commodity &&
              createUniversityBudget(kind, commodity, budget.crop_year)
            }
          />
          <InsuranceCalculator
            workspace={workspace}
        ...
        <>
          {wholeFarm.budgetCount >= 2 && wholeFarm.overlap && (
            <p className="university-note">
              Two of this year's budgets allocate the same field, so whole-farm
              totals would count those acres twice. Keep each field in one plan
              to see farm totals.
            </p>
          )}
          ...
          {wholeFarm.budgetCount >= 2 && !wholeFarm.overlap && (
            ...
            >
              ...
              <WholeFarmKpi
                ...
                value={money.format(wholeFarm.income)}
              />
              <WholeFarmKpi
                label="Total expected costs"
                value={money.format(wholeFarm.costs)}
              />
              <WholeFarmKpi
                label="Total expected profit"
                value={money.format(wholeFarm.profit)}
                alert={wholeFarm.profit < 0}
              />
            </section>
          )}
          <section
            className="overview-budgets"
            aria-labelledby="overview-budgets-title"
          >
            <div className="overview-budgets-heading">
              ...
              <label>
                Crop year
                <select
                  value={currentOverviewYear ?? ""}
                  onChange={(event) => {
                    const year = numberValue(event.target.value);
                    setOverviewYear(year);
                    const inYear = workspace.budgets.filter(
                      (item) => item.crop_year === year,
                    );
                    if (
                      !inYear.some((item) => item.id === selectedId) &&
                      inYear[0]
                    )
                      setSelectedId(inYear[0].id);
                  }}
                >
            ...
            <div className="overview-budget-grid">
              {overviewBudgets.map((item) => (
                <OverviewBudgetCard
                  key={item.id}
                  budget={item}
                  commodity={workspace.fields.commodities.find(
                    (entry) => entry.id === item.commodity_id,
                  )}
                  costs={workspace.cost_lines.filter(
                    (line) => line.budget_id === item.id,
                  )}
                  allocatedAcres={workspace.allocations
                    .filter((allocation) => allocation.budget_id === item.id)
                    .reduce(
                  ...
                  onView={() => {
                    ...
                    navigate("/profitability/budgets");
                  }}
                />
              ))}
            </div>
        ...
        <>
          ...
          {plans.length > 1 && (
            <RoiAnalyzer
              ...
              plans={plans}
              cropKind={cropKind}
              commodityLabel={commodity?.name ?? "this crop"}
            />
          )}
        </>
      )}
      {tabPath === "reports" && (
        <ReportsLauncher
          ...
          budget={budget}
          landlordName={landlordName}
          onLandlordChange={setLandlordName}
          onOpenBanker={() => setReportOpen(true)}
          onOpenLandlord={() => setLandlordReportOpen(true)}
        />
      )}
      {tabPath === "" && (
        <section
          className="profitability-card matrix-card"
        ...
        >
          ...
          <MatrixControls
            ...
            prices={prices}
            yields={yields}
            onSave={(priceSteps, yieldSteps) =>
              save(budget.id, () =>
                profitabilityRepository.replaceMatrixSteps(budget.id, [
                  ...priceSteps,
                  ...yieldSteps,
                ]),
              )
          ...
          <ProfitabilityMatrix
            ...
            yields={yields}
            cost={costsPerAcre}
            minimumRevenueGuaranteePerAcre={
              revenueProtectionMath(budget, costsPerAcre, selectedAcres || null)
                ?.minimumRevenueGuaranteePerAcre ?? null
            }
            selected={selected}
            onPick={setPickedCell}
          />
          <p className="matrix-answer" aria-live="polite">
            At {money.format(selected.price)}/bu and{" "}
            {whole.format(selected.yield)} bu/ac you{" "}
            {selectedProfit >= 0 ? "make" : "lose"}{" "}
            {money.format(Math.abs(selectedProfit))}/ac
            {selectedAcres > 0
              ? `, ${money.format(Math.abs(selectedProfit) * selectedAcres)} on ${decimal.format(selectedAcres)} ac`
              : ""}
            .
          </p>
        </section>
      ...
      {tabPath === "budgets" && (
        <>
          <section
            className="profitability-card costs-card"
          ...
          >
            ...
            <div className="table-scroll">
              <table>
                ...
                <CostLineGroups
                  ...
                  onSave={(next) =>
                    save(budget.id, () => profitabilityRepository.saveCostLine(next))
                  }
                  onRemove={(id) =>
                    save(budget.id, () => profitabilityRepository.deleteCostLine(id))
                  }
                />
              </table>
            </div>
            <div className="cost-add-row">
              <input
                ...
                onChange={(event) =>
                  setNewCost({ ...newCost, name: event.target.value })
                }
              />
              <select
                aria-label="New cost category"
              ...
              <input
                aria-label="New cost dollars per acre"
                inputMode="decimal"
              ...
              <button
                ...
                type="button"
                onClick={() => {
                  const amount = numberValue(newCost.amount);
                  if (
                    !newCost.name.trim() ||
                    !Number.isFinite(amount) ||
                    amount < 0
                  ) {
                    setError("Enter a cost name and a dollar amount per acre.");
                    return;
                  }
                  const at = new Date().toISOString();
                  void save(budget.id, () =>
                    profitabilityRepository.saveCostLine({
                      id: crypto.randomUUID(),
                      budget_id: budget.id,
                    ...
                    saved && setNewCost({ name: "", category: "seed", amount: "" }),
                  );
                }}
              >
                Add cost
            ...
            <EquipmentCostImporter
              budget={budget}
              equipment={workspace.equipment ?? []}
              budgetAcres={selectedAcres}
              onSaved={refresh}
              onError={setError}
            />
          ...
          <FieldAllocation
            ...
            onRemove={(id) =>
              save(budget.id, () => profitabilityRepository.deleteAllocation(id))
            }
          />
        </>
      )}
...
export function EquipmentCostImporter({
  budget,
  equipment,
  budgetAcres,
  onSaved,
  ...
  budget: CropBudget;
  equipment: ProfitabilityEquipment[];
  budgetAcres: number;
  onSaved: () => Promise<void>;
  onError: (message: string) => void;
  repository?: Pick<ProfitabilityRepository, "previewEquipmentCostSnapshot" | "saveEquipmentCostSnapshot">;
}) {
  const defaultStart = `${budget.crop_year}-01-01`;
  const defaultEnd = `${budget.crop_year}-12-31`;
  const [open, setOpen] = useState(false);
  const [equipmentId, setEquipmentId] = useState(equipment[0]?.id ?? "");
  const [periodStart, setPeriodStart] = useState(defaultStart);
  const [periodEnd, setPeriodEnd] = useState(defaultEnd);
  const [allocationAcres, setAllocationAcres] = useState(
  ...
  const confirm = async (action: "insert" | "replace") => {
    ...
    const next: EquipmentCostSnapshotRequest = {
      ...
      expected: {
        ...
        included_row_count: preview.candidate.included_row_count,
  ...
  if (!open)
    return (
      <button className="secondary-action" type="button" onClick={() => setOpen(true)}>
        Import equipment service costs
      </button>
    );
  ...
  return (
    <section className="profitability-card" aria-labelledby="equipment-cost-import-title">
      ...
      {equipment.length === 0 ? (
        <p>Add a machine and service costs on Equipment before importing them here.</p>
      ) : (
        <div className="cost-add-row">
          <label>
            Machine
      ...
      {budgetAcres > 0 && Number(allocationAcres) > budgetAcres && (
        <p role="status" className="university-note">
...
function WholeFarmKpi({
  ...
  return (
    <div className={`whole-farm-kpi${alert ? " alert" : ""}`}>
      <span>{label}</span>
      <strong>{value}</strong>
    </div>
  );
...
function ReportsLauncher({
  ...
  budget: CropBudget;
  landlordName: string;
  onLandlordChange: (name: string) => void;
  onOpenBanker: () => void;
  onOpenLandlord: () => void;
}) {
  ...
  const landlords = [...namesByKey.values()].sort((left, right) =>
    left.localeCompare(right),
  );
  useEffect(() => {
    if (
      landlordName !== "all" &&
      !namesByKey.has(landlordName.trim().toLowerCase())
    )
      onLandlordChange("all");
  }, [landlordName, landlords, onLandlordChange]);
  return (
    <section className="profitability-card reports-card">
      <div className="section-heading">
        <div>
          <span className="eyebrow">Reports</span>
          <h2>Share your numbers</h2>
          <p>Print-ready pages built from the budget picked on Overview.</p>
        </div>
      </div>
      <div className="report-launcher">
        <div className="report-launcher-row">
          <button
            className="primary-action"
            type="button"
            onClick={onOpenBanker}
          >
            Open banker report — {budget.name}
          </button>
          <p>
            Projected cost, break-even, and per-field numbers formatted for a
            lender meeting.
          </p>
        </div>
        <div className="report-launcher-row">
          ...
          <button
            ...
            disabled={!landlords.length}
          >
            Open landlord report
          </button>
          <p>
            {landlords.length
              ? "Planting, yield, applied inputs, and the crop-share settlement for each landlord field."
              : `Add a landlord name to a ${budget.crop_year} field arrangement to print a settlement report.`}
          </p>
        </div>
      </div>
    </section>
...
function OverviewBudgetCard({
  ...
  const nonLandBreakeven =
    costPerAcre - nonLandCostPerAcre(costs) > 0 &&
    budget.expected_yield_per_acre > 0
      ? nonLandCostPerAcre(costs) / budget.expected_yield_per_acre
      : null;
  const topCategories = categories
    .map((category, order) => ({
      ...
      order,
      amount: costs
        .filter((line) => line.category === category.value)
        .reduce((sum, line) => sum + line.amount_per_acre, 0),
    }))
    .filter((category) => category.amount > 0)
    .sort(
      (left, right) => right.amount - left.amount || left.order - right.order,
    )
    .slice(0, 3);
  ...
  return (
    <article className={`overview-budget-card${selected ? " selected" : ""}`}>
      <div
        className="overview-budget-main"
        role="button"
        tabIndex={0}
        aria-pressed={selected}
        aria-label={`Show the ${budget.name} matrix`}
        onClick={onSelect}
        onKeyDown={selectFromKeyboard}
      >
        <header>
          <span>{commodity?.name ?? "Crop"}</span>
          <h3>{budget.name}</h3>
        </header>
        <div
          className={`overview-profit${analysis.expectedProfitPerAcre < 0 ? " negative" : ""}`}
        >
          <span>Profit / ac</span>
          <strong>{money.format(analysis.expectedProfitPerAcre)}</strong>
        </div>
        <dl className="overview-budget-metrics">
          <div>
            <dt>Cost / ac</dt>
            <dd>{money.format(costPerAcre)}</dd>
          </div>
          <div>
            <dt>Break-even price</dt>
            <dd>
              {money.format(analysis.breakevenPricePerBushel)}/bu
              {nonLandBreakeven !== null && (
                <em>{money.format(nonLandBreakeven)}/bu before land</em>
              )}
            </dd>
          </div>
          <div>
            <dt>Break-even yield</dt>
            <dd>{decimal.format(analysis.breakevenYieldPerAcre)} bu/ac</dd>
          </div>
        </dl>
        <div className="overview-cost-bars" aria-label="Top cost categories">
          <span className="overview-cost-bars-title">Top costs</span>
          {topCategories.length > 0 ? (
            topCategories.map((category) => (
              <div key={category.value} className="overview-cost-bar">
                <span>{category.label}</span>
                <div aria-hidden="true">
                  <i
                    style={{
                      width: `${(category.amount / largestCategory) * 100}%`,
                    }}
                  />
                </div>
                <strong>{money.format(category.amount)}/ac</strong>
              </div>
            ))
          ) : (
      ...
      <footer>
        ...
        <button className="overview-view-budget" type="button" onClick={onView}>
          View budget →
        </button>
      </footer>
    </article>
...
function CostLineGroups({
  ...
  return (
    <>
      {categories.map((category) => {
        const lines = costs.filter((line) => line.category === category.value);
        if (lines.length === 0) return null;
        const isCollapsed = collapsed.has(category.value);
        const subtotal = totalCostPerAcre(lines);
        return (
          <tbody key={category.value} className="cost-category-group">
            <tr className="cost-category-subtotal">
              <th scope="rowgroup" colSpan={2}>
                <span>{category.label}</span>
                <small>
                  {whole.format(lines.length)}{" "}
              ...
              <td colSpan={2}>
                ...
                >
                  <span className="sr-only">
                    {isCollapsed
                      ? `Show ${category.label} costs`
                      : `Hide ${category.label} costs`}
                  </span>
...
function BudgetControls({
  ...
  onSave: (budget: CropBudget) => void;
  onCreate: (budget: CropBudget) => void;
  ...
  const update = (patch: Partial<CropBudget>) =>
    onSave({ ...budget, ...patch });
  return (
    <section className="budget-controls profitability-card">
      <label>
        Budget
        <select
          value={selectedId}
      ...
      <label>
        ...
        <input
          ...
          onBlur={(event) =>
            ...
            update({ name: event.target.value.trim() })
          }
        />
      </label>
      <label>
        ...
        <input
          key={`${budget.id}-year`}
          defaultValue={budget.crop_year}
          inputMode="numeric"
          onBlur={(event) => {
            const value = numberValue(event.target.value);
            if (Number.isInteger(value)) update({ crop_year: value });
          }}
        />
      </label>
      <label>
        Commodity
        <select
          value={budget.commodity_id}
          onChange={(event) => update({ commodity_id: event.target.value })}
        >
          {workspace.fields.commodities.map((commodity) => (
      ...
      <label>
        ...
        <input
          key={`${budget.id}-yield`}
          defaultValue={budget.expected_yield_per_acre}
          inputMode="decimal"
          onBlur={(event) => {
            const value = numberValue(event.target.value);
            if (value > 0) update({ expected_yield_per_acre: value });
          }}
        />
      </label>
      <label>
        ...
        <input
          key={`${budget.id}-price`}
          defaultValue={budget.expected_price_per_bushel}
          inputMode="decimal"
          onBlur={(event) => {
            const value = numberValue(event.target.value);
            if (value > 0) update({ expected_price_per_bushel: value });
          }}
        />
      </label>
      <button
...
function InsuranceCalculator({
  ...
  const pricePlaceholder =
    commodity?.id === "corn_yellow"
      ? "2026 RMA projected price $4.62/bu"
      : commodity?.id === "soybeans"
        ? "2026 RMA projected price $11.09/bu"
        : "Enter your projected price";
  const allocatedAcres = workspace.allocations
    .filter((allocation) => allocation.budget_id === budget.id)
    .reduce((total, allocation) => total + allocation.allocated_acres, 0);
  const insurance = revenueProtectionMath(
    budget,
    totalCostPerAcre(costs),
    allocatedAcres || null,
  );
  const insuranceFields = () => ({
    rp_coverage_pct: budget.rp_coverage_pct,
    rp_aph_yield: budget.rp_aph_yield,
    rp_projected_price: budget.rp_projected_price,
    rp_premium_per_acre: budget.rp_premium_per_acre,
  });
  ...
  return (
    <section
      className="profitability-card insurance-card"
    ...
    >
      ...
      <div className="insurance-inputs">
        <label>
          Coverage %
          <select
            value={draft.rp_coverage_pct ?? ""}
            onChange={(event) =>
              update({ rp_coverage_pct: nullableNumber(event.target.value) })
            }
          >
        ...
        <label>
          APH yield (bu/ac)
          <input
            value={draft.rp_aph_yield ?? ""}
            inputMode="decimal"
            placeholder="Enter your APH"
            onChange={(event) =>
              update({ rp_aph_yield: nullableNumber(event.target.value) })
            }
          />
        </label>
        <label>
          Projected price ($/bu)
          <input
            value={draft.rp_projected_price ?? ""}
            inputMode="decimal"
            placeholder={pricePlaceholder}
            onChange={(event) =>
              update({ rp_projected_price: nullableNumber(event.target.value) })
            }
          />
          <small>
        ...
        <label>
          Premium ($/ac) <small>optional</small>
          <input
            value={draft.rp_premium_per_acre ?? ""}
            inputMode="decimal"
            min="0"
            placeholder="Optional"
            onChange={(event) =>
              update({
                rp_premium_per_acre: nullableNumber(event.target.value),
              })
            }
          />
        </label>
...
function MatrixControls({
  ...
  yields: ProfitabilityMatrixStep[];
  onSave: (
    priceSteps: ProfitabilityMatrixStep[],
  ...
  const [values, setValues] = useState(() => ({
    ...
    pMax: prices.at(-1)?.value ?? 6,
    pStep: prices.length > 1 ? prices[1].value - prices[0].value : 0.25,
    yMin: yields[0]?.value ?? 150,
    yMax: yields.at(-1)?.value ?? 250,
    yStep: yields.length > 1 ? yields[1].value - yields[0].value : 10,
  }));
  useEffect(
    () =>
      setValues({
  ...
  return (
    <div className="matrix-controls">
      ...
      <input
        ...
        inputMode="decimal"
        onChange={(event) =>
          setValues({ ...values, pMin: numberValue(event.target.value) })
        }
      />
      <input
        ...
        inputMode="decimal"
        onChange={(event) =>
          setValues({ ...values, pMax: numberValue(event.target.value) })
        }
      />
      <input
        aria-label="Price step"
        value={values.pStep}
        inputMode="decimal"
        onChange={(event) =>
      ...
      <input
        ...
        inputMode="decimal"
        onChange={(event) =>
          setValues({ ...values, yMin: numberValue(event.target.value) })
        }
      />
      <input
        aria-label="Highest yield"
        value={values.yMax}
        inputMode="decimal"
        onChange={(event) =>
          setValues({ ...values, yMax: numberValue(event.target.value) })
        }
      />
      <input
        aria-label="Yield step"
        value={values.yStep}
        inputMode="decimal"
        onChange={(event) =>
          setValues({ ...values, yStep: numberValue(event.target.value) })
        }
      />
      <button
        className="secondary-action"
        ...
        onClick={() => {
          try {
            ...
            onError(
              caught instanceof Error
                ? caught.message
                : "Enter a sensible low, high, and step for the matrix.",
            );
          }
        }}
      >
...
function ProfitabilityMatrix({
  ...
  yields: ProfitabilityMatrixStep[];
  cost: number;
  minimumRevenueGuaranteePerAcre: number | null;
  selected: { price: number; yield: number };
  onPick: (cell: { price: number; yield: number }) => void;
}) {
  const cells = matrixCells(prices, yields, cost);
  const max = Math.max(...cells.map((cell) => Math.abs(cell.profit)), 1);
        ...
        <table className="profit-matrix">
          ...
          <tbody>
            {yields
              ...
              .map((yieldStep) => (
                <tr key={yieldStep.id}>
                  <th>{whole.format(yieldStep.value)} bu</th>
                  {prices.map((price) => {
                    const profit = matrixProfitPerAcre(
                      price.value,
                      yieldStep.value,
                      cost,
                    );
                    const nearZero = breakeven.has(
                      `${price.value}|${yieldStep.value}`,
                    );
                    const active =
                      selected.price === price.value &&
                      selected.yield === yieldStep.value;
                    const belowInsurance =
                      minimumRevenueGuaranteePerAcre !== null &&
                      price.value * yieldStep.value <
                        minimumRevenueGuaranteePerAcre;
                    return (
                      <td
                        key={price.id}
                        className={
                          belowInsurance ? "below-insurance-floor" : ""
                        }
                      >
                        <button
                          type="button"
                          className={`${nearZero ? "breakeven-cell " : ""}${active ? "active-cell" : ""}`}
                          style={{ backgroundColor: matrixColor(profit, max) }}
                          onClick={() =>
                            onPick({
                              price: price.value,
    ...
    return (
      <tr>
        <td>
          ...
          {line.source_kind === "equipment" && line.equipment_snapshot && (
            <small>
              Snapshot {line.equipment_snapshot.period_start} to {line.equipment_snapshot.period_end}: {money.format(line.equipment_snapshot.total_source_amount)} across {decimal.format(line.equipment_snapshot.allocation_acres)} acres. {line.equipment_snapshot.included_row_count} costed service {line.equipment_snapshot.included_row_count === 1 ? "entry" : "entries"}; {line.equipment_snapshot.excluded_null_cost_count} without cost excluded. Saved {new Date(line.equipment_snapshot.captured_at).toLocaleString()}.
            </small>
          )}
        </td>
        <td>
    ...
    <tr>
      <td>
        <input
          ...
          onBlur={(event) =>
            event.target.value.trim() &&
            onSave({ ...line, name: event.target.value.trim() })
          }
        />
        {defaultAmount !== undefined &&
          defaultAmount === line.amount_per_acre && (
      ...
      <td>
        <select
          ...
          onBlur={(event) =>
            onSave({ ...line, category: event.target.value as CostCategory })
          }
        >
          {categories.map((category) => (
            <option key={category.value} value={category.value}>
      ...
      <td className="numeric">
        <input
          ...
          onBlur={(event) => {
            const value = numberValue(event.target.value);
            if (value >= 0) onSave({ ...line, amount_per_acre: value });
          }}
        />
      </td>
      <td className="numeric">
...
function FieldAllocation({
  ...
  costs: BudgetCostLine[];
  saving: boolean;
  onSave: (allocation: BudgetFieldAllocation) => void;
  onRemove: (id: string) => void;
}) {
  const assignments = workspace.fields.crop_assignments.filter(
    (item) =>
      item.crop_year === budget.crop_year &&
      item.commodity_id === budget.commodity_id,
  );
  const allocations = workspace.allocations.filter(
    (item) => item.budget_id === budget.id,
  );
  const allocatedAcres = allocations.reduce(
    (sum, item) => sum + item.allocated_acres,
    0,
  );
  const plantedAcres = assignments.reduce(
    (sum, item) => sum + item.planted_acres,
    0,
  );
  const allocatedTotalCost = allocations.reduce(
    (total, allocation) =>
      total +
      (allocationFinancials(workspace, budget, costs, allocation)?.costTotal ??
        0),
    0,
  );
  return (
    <section
      className="profitability-card allocation-card"
      aria-labelledby="field-cost-title"
    >
      <div className="section-heading">
        <div>
      ...
      {plantedAcres > allocatedAcres + 0.01 && (
        <p className="allocation-nudge">
          {decimal.format(plantedAcres - allocatedAcres)} planted ac are not
          allocated yet. Pick a field below when you are ready.
        </p>
      )}
      <div className="allocation-list">
        {assignments.map((assignment) => {
          const allocation = allocations.find(
            (item) => item.crop_assignment_id === assignment.id,
          );
          ...
          if (!allocation)
            return (
              <div className="allocation-row" key={assignment.id}>
                <div>
                  <strong>{field.name}</strong>
                  <span>
                    {decimal.format(assignment.planted_acres)} planted ac
                  </span>
                </div>
                <button
                  className="secondary-action"
                  ...
                  onClick={() => {
                    ...
                    onSave({
                      ...
                      allocated_acres: assignment.planted_acres,
                      expected_yield_override: null,
                      expected_price_override: null,
                      created_at: at,
                      updated_at: at,
                    });
                  }}
          ...
          const figures = allocationFinancials(
            workspace,
            budget,
            costs,
            allocation,
            ...
            <div className="allocated-field" key={allocation.id}>
              <div className="field-edit">
                ...
                <label>
                  Acres
                  <input
                    defaultValue={allocation.allocated_acres}
                    inputMode="decimal"
                    disabled={saving}
                    onBlur={(event) => {
                      const value = numberValue(event.target.value);
                      if (value > 0 && value <= assignment.planted_acres)
                        onSave({ ...allocation, allocated_acres: value });
                    }}
                  />
                </label>
                <label>
                  Yield
                  <input
                    defaultValue={figures.yieldPerAcre}
                    inputMode="decimal"
                    disabled={saving}
                    onBlur={(event) => {
                      const value = numberValue(event.target.value);
              ...
              <div className="field-results">
                <span>
                  Cost/ac <strong>{money.format(figures.costPerAcre)}</strong>
                </span>
                <span>
                  Total cost <strong>{money.format(figures.costTotal)}</strong>
                </span>
                <span>
                  Net/ac{" "}
                  <strong
                    className={figures.profitPerAcre < 0 ? "negative" : ""}
                  >
                    {money.format(figures.profitPerAcre)}
                  </strong>
                </span>
                <span>
                  Total net{" "}
                  <strong className={figures.profitTotal < 0 ? "negative" : ""}>
                    {money.format(figures.profitTotal)}
                  </strong>
                </span>
                <button
                  className="remove-cost"
...
export function wholeFarmTotals(
  ...
  for (const budget of budgets) {
    ...
    const allocations = workspace.allocations.filter(
      (allocation) => allocation.budget_id === budget.id,
    );
...
function PlanComparison({
  ...
  if (plans.length < 2)
    return (
      <section className="profitability-card plan-compare-card">
        <div className="section-heading">
          <div>
            <span className="eyebrow">Compare plans</span>
            <h2>Try a second plan side by side</h2>
            <p>
              Copy this budget into a "Cheap" or "Full program" version and
              change a few costs. The comparison shows up here with a winner for
              every land arrangement.
            </p>
          </div>
        </div>
      </section>
  ...
  const asBudgeted = plans.map((plan) =>
  ...
  return (
    <section
      className="profitability-card plan-compare-card"
        ...
        <table className="plan-compare">
          ...
          <tbody>
            {plans.map((plan, planIndex) => {
              const lines = linesFor(plan.id);
              const cost = totalCostPerAcre(lines);
              const planNumbers = budgetAnalysis(plan, cost);
              const cushions = planCushions(plan, lines);
              return (
                <tr
                  key={plan.id}
                  className={plan.id === selected.id ? "active-plan" : ""}
                >
                  <th scope="row">
                    <button
                      type="button"
                    ...
                    <span className="plan-scenario">
                      {money.format(plan.expected_price_per_bushel)}/bu ·{" "}
                  ...
                  {arrangementColumns.map((column, columnIndex) => {
                    ...
                    return (
                      ...
                      >
                        {value === null
                          ...
                          : money.format(value)}
                        {value !== null &&
                          value === arrangementBest[columnIndex] && (
                            <span className="winner-badge">Best</span>
                          )}
                      </td>
                    );
...
function RoiAnalyzer({
  ...
  const byCost = plans
    .slice()
    .sort((left, right) => inputCost(right) - inputCost(left));
  ...
  useEffect(() => {
    ...
    if (
      !ids.has(spendMoreId) ||
      !ids.has(cheaperId) ||
      spendMoreId === cheaperId
    ) {
      const sorted = plans
        .slice()
        .sort((left, right) => inputCost(right) - inputCost(left));
      setSpendMoreId(sorted[0].id);
      setCheaperId(sorted[1].id);
    }
  }, [plans.map((plan) => plan.id).join("|")]);
  const spendMore = plans.find((plan) => plan.id === spendMoreId) ?? byCost[0];
  const cheaper =
    plans.find((plan) => plan.id === cheaperId && plan.id !== spendMore.id) ??
    byCost.find((plan) => plan.id !== spendMore.id)!;
  ...
  const whatIf =
    extraYield.trim() !== "" && Number.isFinite(extra) && extra >= 0
      ? roiWhatIfPerAcre(difference, extra, price)
      : null;
  const picker = (
    label: string,
    value: string,
      ...
      <select value={value} onChange={(event) => onChange(event.target.value)}>
        {plans.map((plan) => (
          <option key={plan.id} value={plan.id}>
            {plan.name} ({money.format(inputCost(plan))}/ac inputs)
          </option>
        ))}
      </select>
    </label>
  ...
  return (
    <section
      className="profitability-card roi-card"
    ...
    >
      ...
      <div className="roi-controls">
        {picker("Spend-more plan", spendMore.id, setSpendMoreId)}
        {picker("Cheaper plan", cheaper.id, setCheaperId)}
        <div className="roi-diff">
          <span>Extra spend</span>
          <strong>{money.format(difference)}/ac</strong>
        </div>
      </div>
      {difference <= 0 ? (
        <p className="roi-guide">
          Your spend-more plan does not actually cost more on non-land inputs —
          it wins at any yield. Swap the two plans to test the other direction.
        </p>
      ) : (
        <>
          <div className="table-scroll">
            <table className="plan-compare roi-table">
              ...
              <tbody>
                {roiPriceLadder(price).map((ladderPrice) => {
          ...
          <div className="roi-whatif">
            <label>
              Extra bushels you expect from {spendMore.name}
              <input
                inputMode="decimal"
                value={extraYield}
                placeholder="bu/ac"
                onChange={(event) => setExtraYield(event.target.value)}
              />
            </label>
            {whatIf !== null && (
              <span
                className={`roi-verdict${whatIf < 0 ? " negative" : ""}`}
                aria-live="polite"
              >
                {whatIf >= 0
                  ? `${spendMore.name} wins by ${money.format(whatIf)}/ac`
                  : `${cheaper.name} wins by ${money.format(Math.abs(whatIf))}/ac`}
              </span>
            )}
          </div>
          <p className="roi-guide">
            Verdict scale for {cropKind === "soybeans" ? "soybeans" : "corn"}: ≤
            {thresholds.easyYes} bu easy yes · ≤{thresholds.likely} bu likely
            worth it · ≤{thresholds.marginal} bu marginal. General guidance only
            —{" "}
            {cropKind === "soybeans"
              ? "fungicide alone is typically a 2–4 bu edge, premium seed treatment 1–3 bu, full vs basic 1–4 bu"
              : "fungicide alone is typically an 8–15 bu edge, premium seed treatment 3–8 bu, full vs stripped program 10–17 bu"}
            ; your results depend on weather, disease pressure, and soil.
          </p>
        </>
      )}
    </section>
...
function CoachNudge({
  ...
  costs: BudgetCostLine[];
  cropKind: FarmdocCropKind | null;
  onAdd: (lines: BudgetCostLine[]) => void;
}) {
  ...
  const suggestions = cropKind
    ? missing
        .map((category) => farmdocTypicalLine(cropKind, category))
        .filter((line): line is NonNullable<typeof line> => line !== null)
    : [];
  const dismiss = () => {
    const next = [...dismissed, budget.id];
    setDismissed(next);
    try {
  ...
  return (
    <div className="allocation-nudge coach-nudge">
      <p>
        What am I forgetting? Full budgets usually also have:{" "}
        {suggestions.length > 0
          ? suggestions
              .map((line) => `${line.name} (~$${line.amount_per_acre}/ac)`)
              .join(", ")
          : missing
              .map(
                (category) =>
              ...
              .join(", ")}
        . Skipping them makes your break-even look better than it is.
      </p>
      <div className="coach-actions">
        {suggestions.length > 0 && (
          <button
            ...
            onClick={() => {
              const at = new Date().toISOString();
              const lines: BudgetCostLine[] = suggestions.map((line) => ({
                id: crypto.randomUUID(),
                budget_id: budget.id,
                category: line.category,
                name: line.name,
        ...
        <button className="remove-cost" type="button" onClick={dismiss}>
          Dismiss
        </button>
      </div>
    </div>
      ...
      <div>
        {rows.map(([type, value]) => (
          <span
            key={type}
            className={
              arrangement?.arrangement_type === type ? "active-arrangement" : ""
            }
          >
            <strong>{arrangementLabels[type]}</strong>
            <b>
              {value === null
                ? "Set up in Fields"
                : `${money.format(value)}/ac`}
            </b>
          </span>
        ))}
      </div>
