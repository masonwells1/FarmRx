...
export type GrainCarryPersistence = { saveSettings: (settings: GrainCarrySettings) => Promise<void>; saveGrid: (grid: GrainCarryGrid) => Promise<void>; createId: () => string }
...
export function GrainCostOfCarry({ workspace, selectedEstimate, selectedEstimateId, onSelectEstimate, persistence }: { workspace: GrainWorkspace; selectedEstimate: ProductionEstimate; selectedEstimateId: string; onSelectEstimate: (id: string) => void; persistence?: GrainCarryPersistence }) {
  ...
  const flushSettings = () => {
    ...
    enqueue(async () => { const current = workspaceRef.current; await persistenceRef.current?.saveSettings(settingsToRow(current.fields.farm.id, snapshot, current.grain_carry_settings?.updated_at ?? new Date().toISOString())) })
  }
  const flushGrids = () => {
    ...
    for (const estimateId of ids) {
      ...
      enqueue(async () => {
        ...
        await active.saveGrid(carryToGrid(snapshot, { id, farm_id: current.fields.farm.id, production_estimate_id: estimateId, updated_at: existing?.updated_at ?? new Date().toISOString() }))
      })
    }
  }
  useEffect(() => { setSettings(initialSettings()); setByEstimate(initialGrids()); settingsDirty.current = false; gridsDirty.current.clear() }, [farmId, persisted]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const storageNote = persisted ? 'Your rates and prices are saved for this farm on every device.' : 'Your rates and prices stay on this device until the farm settings update is live.'
