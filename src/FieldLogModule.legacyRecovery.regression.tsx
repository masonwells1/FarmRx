...
const user = '00000000-0000-4000-8000-000000000001'; const farm = '00000000-0000-4000-8000-000000000002'; const field = '00000000-0000-4000-8000-000000000003'; const operation = '00000000-0000-4000-8000-000000000004'; const entry = '00000000-0000-4000-8000-000000000005'; const rainOperation = '00000000-0000-4000-8000-000000000006'; const rainEntry = '00000000-0000-4000-8000-000000000007'; const stamp = '2027-08-04T19:00:00.000Z'; const queueKey = fieldLogWriteQueueKey('pine-proof', user, farm)
...
const legacyZeroRain = { ...legacy, operationId: rainOperation, draft: { id: rainEntry, field_id: field, entry_type: 'rainfall' as const, observed_on: '2027-08-05', rainfall_in: 0, note: null } }
...
try {
  ...
  for (const expected of ['Saved changes that need attention', 'Field log entry', '2027-08-04 · Synthetic legacy note', '2027-08-05 · 0 in', 'cannot be sent automatically', 'Re-enter this change manually if needed, then dismiss it.', 'Review only', 'Dismiss', 'Pine North 60']) assert(text.includes(expected), `Missing legacy recovery detail: ${expected}`)
