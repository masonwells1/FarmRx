...
const initialForm = () => ({ fieldId: '', sampleDate: '', labName: '', measurements: emptyMeasurements() })
...
function fileError(file: File) { const allowed = ['application/pdf', 'image/jpeg', 'image/png', 'image/heic', 'image/heif']; if (!allowed.includes(file.type.toLowerCase())) return 'Choose a PDF, JPEG, PNG, HEIC, or HEIF report.'; if (file.size > 20 * 1024 * 1024) return 'Choose a report smaller than 20 MB.'; return null }
...
export function SoilRxPage({ repository, fieldsRepository }: { repository: SoilRxRepository; fieldsRepository: FieldsRepository }) {
  ...
  const refresh = async () => {
    const [fieldData, soilData] = await Promise.all([fieldsRepository.getData(), repository.getData()])
    ...
    setSelectedFieldId((current) => current && nextFields.some((field) => field.id === current) ? current : nextFields[0]?.id ?? '')
    setForm((current) => current.fieldId && nextFields.some((field) => field.id === current.fieldId) ? current : { ...current, fieldId: nextFields[0]?.id ?? '' })
    setOpenTests((current) => current.size ? current : new Set(nextTests.filter((test) => test.field_id === (nextFields[0]?.id ?? '')).slice(0, 1).map((test) => test.id)))
  }
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      const draft: SoilTestDraft = { field_id: form.fieldId, sample_date: form.sampleDate, lab_name: form.labName, ...Object.fromEntries(soilMeasurementKeys.map((key) => [key, form.measurements[key] === '' ? null : Number(form.measurements[key])])) as Record<SoilMeasurementKey, number | null> }
      ...
      setSelectedFieldId(saved.field_id); setForm({ ...initialForm(), fieldId: saved.field_id, sampleDate: farmLocalCalendarDate() }); setReport(null)
  ...
  function selectReport(event: ChangeEvent<HTMLInputElement>) { const file = event.target.files?.[0] ?? null; const problem = file ? fileError(file) : null; setReport(problem ? null : file); setError(problem) }
    ...
    {loading ? <p className="loading-state">Loading Soil Rx…</p> : <>
      {!fields.length ? <p className="soil-rx-empty">Add a field before saving a soil test.</p> : <>
        <div className="soil-rx-layout">
          <aside className="soil-rx-fields" aria-label="Fields"><h2>Your fields</h2>{fields.map((field) => <button key={field.id} type="button" className={field.id === selectedField ? 'active' : ''} onClick={() => { setSelectedFieldId(field.id); setForm((current) => ({ ...current, fieldId: field.id })); setOpenTests(new Set(sortSoilTestsNewestFirst(tests.filter((test) => test.field_id === field.id)).slice(0, 1).map((test) => test.id))) }}>{field.name}<small>{tests.filter((test) => test.field_id === field.id).length} tests</small></button>)}</aside>
