...
export class SupabaseScoutingDataGateway implements ScoutingDataGateway {
  ...
  async saveNote(input: { farmId: string; operationId: string; note: ScoutingNoteDraft }) { const { id, field_id, observed_on, category, note, latitude, longitude, photos, create_task } = input.note; const p_note = { ...(id ? { id } : {}), field_id, observed_on, category, note, latitude, longitude, photos, create_task }; const result = await supabase.rpc('save_scouting_note', { p_farm_id: input.farmId, p_operation_id: input.operationId, p_note }); return row(result.data, result.error) }
  async deleteNote(input: { farmId: string; noteId: string }) { const result = await supabase.rpc('delete_scouting_note', { p_farm_id: input.farmId, p_note_id: input.noteId }); return row(result.data, result.error) }
}
