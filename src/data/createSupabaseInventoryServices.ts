...
export function createSupabaseInventoryServices(dependencies: { fieldsRepository: FieldsRepository; getFarmId: () => Promise<string>; getContext: () => Promise<{ userId: string; farmId: string }>; projectRef: string; storage: StorageLike; createId: () => string; isOffline: () => boolean }) {
  ...
  const live = new SupabaseInventoryRepository({ gateway: new SupabaseInventoryDataGateway(), fieldsRepository: dependencies.fieldsRepository, getFarmId: dependencies.getFarmId, getOperationContext, verifyOperationContext, verifySnapshotContext: (expected) => verifyFarmOperationContext(dependencies.storage, expected, captureFarmOperationContext(dependencies.storage, dependencies.projectRef, { userId: expected.userId, farmId: expected.farmId })), createId: dependencies.createId, clock: () => new Date().toISOString() })
