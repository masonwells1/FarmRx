...
export function createSupabaseGrainServices(dependencies: { fieldsRepository: FieldsRepository; profitabilityRepository: ProfitabilityRepository; getFarmId: () => Promise<string>; getContext: () => Promise<{ userId: string; farmId: string }>; projectRef: string; storage: StorageLike; isOffline: () => boolean }) {
  ...
  const live = new SupabaseGrainRepository({ gateway: new SupabaseGrainDataGateway(), fieldsRepository: dependencies.fieldsRepository, getFarmId: dependencies.getFarmId, getOperationContext, verifyOperationContext, verifySnapshotContext: (expected) => verifyFarmOperationContext(dependencies.storage, expected, captureFarmOperationContext(dependencies.storage, dependencies.projectRef, { userId: expected.userId, farmId: expected.farmId })), createId: createGrainId, clock: () => new Date().toISOString() })
