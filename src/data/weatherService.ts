...
/** Whether a wall-clock time falls between the day's sunrise and sunset; false when either is unknown. */
export function daylight(time: string, sunrise: string | null, sunset: string | null) { if (!sunrise || !sunset) return false; const value = at(time); const start = at(sunrise); const end = at(sunset); return Number.isFinite(value) && Number.isFinite(start) && Number.isFinite(end) && value >= start && value <= end }
