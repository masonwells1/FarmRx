...
async function run() {
  ...
  const offsetStorage = new Storage(); const withOffset = await createWeatherService({ storage: offsetStorage, clock: () => new Date('2026-07-12T15:00:00Z'), fetch: async () => response({ ...payload, utc_offset_seconds: -18000 }) }).fetchForecast(38, -88)
  assert(withOffset.utc_offset_seconds === -18000, 'The provider offset is kept on the live bundle.')
  ...
  assert(cachedWithOffset.utc_offset_seconds === -18000, 'The cached bundle carries the offset too.')
  ...
  assert(!('utc_offset_seconds' in withoutOffset), 'A response without the offset yields a bundle without it.')
