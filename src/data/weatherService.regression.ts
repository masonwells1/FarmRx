...
async function run() {
  ...
  // FD-1 (FD-021): the provider's UTC offset is kept on the bundle and in the cache so Today can place the field's clock at any
  // instant; a response without it yields a bundle without it rather than a guess.
  const offsetStorage = new Storage(); const withOffset = await createWeatherService({ storage: offsetStorage, clock: () => new Date('2026-07-12T15:00:00Z'), fetch: async () => response({ ...payload, utc_offset_seconds: -18000, timezone: 'America/Chicago' }) }).fetchForecast(38, -88)
  assert(withOffset.utc_offset_seconds === -18000 && withOffset.timezone === 'America/Chicago', 'The provider offset and zone are kept on the live bundle.')
  const cachedWithOffset = await createWeatherService({ storage: offsetStorage, clock: () => new Date('2026-07-12T15:10:00Z'), fetch: async () => { throw new Error('offline') } }).fetchForecast(38, -88)
  assert(cachedWithOffset.utc_offset_seconds === -18000 && cachedWithOffset.timezone === 'America/Chicago', 'The cached bundle carries the offset and zone too.')
  const withoutOffset = await createWeatherService({ storage: new Storage(), clock: () => new Date('2026-07-12T15:00:00Z'), fetch: async () => response(payload) }).fetchForecast(38, -88)
  assert(!('utc_offset_seconds' in withoutOffset) && !('timezone' in withoutOffset), 'A response without the offset or zone yields a bundle without them.')
  ...
  const boundaryStorage = new Storage(); const boundaryStart = new Date('2026-07-12T15:00:00Z'); let boundaryFetches = 0; const boundaryLive = createWeatherService({ storage: boundaryStorage, clock: () => boundaryStart, fetch: async () => { boundaryFetches += 1; return response(payload) } }); await boundaryLive.fetchForecast(38, -88)
  const justFresh = await createWeatherService({ storage: boundaryStorage, clock: () => new Date(boundaryStart.getTime() + 30 * 60 * 1000 - 1), fetch: async () => { boundaryFetches += 1; return response(payload) } }).fetchForecast(38, -88); assert(!justFresh.stale && boundaryFetches === 1, 'A cache one millisecond inside the 30-minute boundary must remain current.')
  await createWeatherService({ storage: boundaryStorage, clock: () => new Date(boundaryStart.getTime() + 30 * 60 * 1000), fetch: async () => { boundaryFetches += 1; return response(payload) } }).fetchForecast(38, -88); assert(Number(boundaryFetches) === 2, 'A cache at the exact 30-minute boundary must refresh.')
  const fallbackBoundaryStorage = new Storage(); await createWeatherService({ storage: fallbackBoundaryStorage, clock: () => boundaryStart, fetch: async () => response(payload) }).fetchForecast(38, -88)
  const fallbackAtCap = await createWeatherService({ storage: fallbackBoundaryStorage, clock: () => new Date(boundaryStart.getTime() + sprayJudgmentMaxAgeMs), fetch: async () => response(emptyHourly) }).fetchForecast(38, -88); assert(fallbackAtCap.stale, 'A cache at the exact two-hour fallback ceiling must remain an explicit stale fallback.')
  const futureStorage = new Storage(); await createWeatherService({ storage: futureStorage, clock: () => new Date(boundaryStart.getTime() + 60_000), fetch: async () => response(payload) }).fetchForecast(38, -88)
  let futureRefreshes = 0; const afterRollback = await createWeatherService({ storage: futureStorage, clock: () => boundaryStart, fetch: async () => { futureRefreshes += 1; return response(payload) } }).fetchForecast(38, -88); assert(!afterRollback.stale && futureRefreshes === 1, 'A future forecast cache timestamp must refresh after clock rollback, never render current.')
  const futureFallbackStorage = new Storage(); await createWeatherService({ storage: futureFallbackStorage, clock: () => new Date(boundaryStart.getTime() + 60_000), fetch: async () => response(payload) }).fetchForecast(38, -88)
  await rejects(() => createWeatherService({ storage: futureFallbackStorage, clock: () => boundaryStart, fetch: async () => response(emptyHourly) }).fetchForecast(38, -88), 'A future forecast cache timestamp must never be accepted as a stale fallback.')
  ...
  let precisionId = 90; const precisionCalls: Array<{ latitude: number; longitude: number }> = []
  const precisionClient = createFieldLocationClient({ gateway: { setFieldLocation: async (input) => { precisionCalls.push({ latitude: input.latitude, longitude: input.longitude }); return { id: input.fieldId, farm_id: input.farmId, latitude: input.latitude.toFixed(6), longitude: input.longitude.toFixed(6), location_source: input.source } } }, getContext: async () => ({ userId: id(4), farmId }), projectRef: 'precision', storage: new Storage(), createId: () => id(precisionId++), clock: () => '2026-07-12T15:00:00.000Z', isOffline: () => false })
  const roundedLocation = await precisionClient.saveLocation(fieldId, 38.1234567, -88.7654321, 'gps')
  assert(precisionCalls[0]?.latitude === 38.123457 && precisionCalls[0].longitude === -88.765432 && roundedLocation?.latitude === 38.123457 && roundedLocation.longitude === -88.765432, 'High-precision GPS coordinates were not normalized to the database precision before save confirmation.')
  ...
  const legacyProject = 'legacy-precision'; const legacyUser = id(6); const legacyStorage = new Storage(); const legacyQueueKey = `farm-rx-field-location-queue:v1:${legacyProject}:${legacyUser}:${farmId}`; const legacyCalls: Array<{ latitude: number; longitude: number }> = []
  legacyStorage.setItem(legacyQueueKey, JSON.stringify({ version: 1, entries: [{ version: 1, module: 'weather', kind: 'setFieldLocation', operationId: id(91), userId: legacyUser, farmId, fieldId, latitude: 38.1234567, longitude: -88.7654321, source: 'gps', enqueuedAt: '2026-07-12T14:00:00.000Z' }] }))
  const legacyClient = createFieldLocationClient({ gateway: { setFieldLocation: async (input) => { legacyCalls.push({ latitude: input.latitude, longitude: input.longitude }); return { id: input.fieldId, farm_id: input.farmId, latitude: input.latitude.toFixed(6), longitude: input.longitude.toFixed(6), location_source: input.source } } }, getContext: async () => ({ userId: legacyUser, farmId }), projectRef: legacyProject, storage: legacyStorage, createId: next, clock: () => '2026-07-12T15:00:00.000Z', isOffline: () => false })
  await legacyClient.replay()
  assert(legacyCalls[0]?.latitude === 38.123457 && legacyCalls[0].longitude === -88.765432 && JSON.parse(legacyStorage.getItem(legacyQueueKey) ?? '{}').entries?.length === 0, 'A legacy high-precision GPS queue did not normalize, confirm, and clear during replay.')
  ...
  const futureHistoryStorage = new Storage(); await createWeatherService({ storage: futureHistoryStorage, clock: () => new Date(historyNow.getTime() + 60_000), fetch: async () => response(fullHistory) }).fetchDailyHistory(38, -88, '2026-07-05', '2026-07-12')
  await rejects(() => createWeatherService({ storage: futureHistoryStorage, clock: () => historyNow, fetch: async () => response(historyPayload(['2026-07-07'])) }).fetchDailyHistory(38, -88, '2026-07-05', '2026-07-12'), 'A future weather-history cache timestamp must never be accepted as a stale fallback.')
  ...
  assert(isActionablyFresh({ stale: false, fetched_at: '2026-07-14T12:00:00Z' }, gateNow) === true, 'A zero-age bundle is actionable.')
  assert(isActionablyFresh({ stale: false, fetched_at: '2026-07-14T10:00:00Z' }, gateNow) === true, 'A bundle at the exact two-hour spray-judgment ceiling is actionable.')
  ...
  assert(isActionablyFresh({ stale: false, fetched_at: '2026-07-14T12:00:01Z' }, gateNow) === false, 'A future bundle must never be actionable after clock rollback.')
