...
async function run() {
  ...
  const artifactReplacementMutations = [
    ...
    { name: 'tracked-only artifact recipe custody removed', index: 9, from: 'evidence.files and evidence.combined_sha256 are historical untracked records only; current proof requires the tracked build-input pins and fresh reusable-artifact inspection.', to: 'CW2_REMOVED_TRACKED_ARTIFACT_RECIPE_CUSTODY' },
  ] as const
  assert(artifactReplacementMutations.length === 21, 'The artifact replacement mutation matrix must retain every stale-identity, canonical identity, tracked-only custody boundary and recipe, provenance, complete build-input evidence, derived-image proof, reusable postcleanup attestation, NUL-safe discovery recipe, standalone attestation, absence, and exact cleanup guard.')
