...
assert(grainModule.includes('movementUnconfirmed ? "Retry movement" : "Add movement"') && (grainModule.match(/disabled=\{movementUnconfirmed\}/g) ?? []).length === 5, 'An uncertain movement must label its exact-payload retry and disable all five editable controls.')
