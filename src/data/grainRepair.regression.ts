...
assert(movementSubmit.includes('movementDraft.current ??= transaction') && movementSubmit.indexOf('await onSave(transaction);') < movementSubmit.indexOf('movementDraft.current = null;'), 'The complete movement draft must remain byte-stable until the saved row is confirmed by refresh.')
assert(grainModule.includes('movementUnconfirmed ? "Retry movement" : "Add movement"') && (grainModule.match(/disabled=\{movementUnconfirmed\}/g) ?? []).length === 6, 'An uncertain movement must label its exact-payload retry and disable all six editable controls, crop year included (LD-5).')
...
const movementOwner = grainModule.slice(grainModule.indexOf('const addMovement = async (transaction: BinTransaction)'), grainModule.indexOf('return (', grainModule.indexOf('const addMovement = async (transaction: BinTransaction)')))
assert(movementOwner.indexOf('setSaveReceipt(transaction.id, "confirmation needed")') < movementOwner.indexOf('This bin movement may be recorded but could not be confirmed.'), 'A confirmed movement write with failed owning refresh must publish Confirmation needed before surfacing the same-movement retry message.')
