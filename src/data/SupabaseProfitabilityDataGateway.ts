...
export class SupabaseProfitabilityDataGateway implements ProfitabilityDataGateway {
  ...
  async patchBudgetInsurance(farmId: string, budgetId: string, patch: InsuranceBudgetPatch) { const result = await supabase.from('crop_budgets').update(insuranceColumns(patch)).eq('farm_id', farmId).eq('id', budgetId).select('*').single(); return row(result.data, result.error) }
