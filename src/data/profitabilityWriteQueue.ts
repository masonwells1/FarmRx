...
const costLineKeys = ['id', 'budget_id', 'category', 'name', 'amount_per_acre', 'sort_order', 'created_at', 'updated_at']
function costLine(value: unknown): value is BudgetCostLineWrite { return record(value) && (exact(value, costLineKeys) || (exact(value, [...costLineKeys, 'university_default_amount']) && (value.university_default_amount === null || (finite(value.university_default_amount) && (value.university_default_amount as number) >= 0)))) && isId(value.id) && isId(value.budget_id) && categories.has(String(value.category)) && typeof value.name === 'string' && value.name.trim().length > 0 && finite(value.amount_per_acre) && Number.isInteger(value.sort_order) && (value.sort_order as number) >= 0 && stamp(value.created_at) && stamp(value.updated_at) }
