...
async function run() {
  ...
  const replayGateway = new FakeGateway(); const replay = new DueProgramItemsService({ gateway: replayGateway, getFarmId: async () => farm, createId: () => uid(10), today: () => '2026-07-12' })
  ...
  let next = 20; const dedupeGateway = new FakeGateway(); const dedupe = new DueProgramItemsService({ gateway: dedupeGateway, getFarmId: async () => farm, createId: () => uid(next++), today: () => '2026-07-12' })
  ...
  const failingGateway = new FakeGateway(); failingGateway.failure = new Error('offline'); const bestEffort = new DueProgramItemsService({ gateway: failingGateway, getFarmId: async () => farm, createId: () => uid(30), today: () => '2026-07-12' })
