...
class FakeGateway implements HarvestDataGateway {
  role: unknown = { role: 'worker' }; saves: Array<{ operationId: string; entry: HarvestDraft }> = []; entries = new Map<string, Record<string, unknown>>(); receipts = new Map<string, unknown>(); mutate: (value: unknown) => unknown = (value) => value; failure: Error | null = null; beforeSave: (() => Promise<void>) | null = null; terminalBeforePersist: Error | null = null; throwAfterPersist = false; afterPersist: (() => void) | null = null; persists = 0
  ...
  async saveHarvest(input: { farmId: string; operationId: string; entry: HarvestDraft }) {
    ...
    if (this.terminalBeforePersist) throw this.terminalBeforePersist
    ...
    this.entries.set(input.entry.crop_assignment_id, result); this.receipts.set(input.operationId, result); this.persists += 1
    if (this.throwAfterPersist) { this.afterPersist?.(); throw new TypeError('fetch failed after the durable write') }
...
async function run() {
  ...
  const gateway = new FakeGateway(); const unrelatedCropId = uid(7); const unrelatedBefore = canonical({ ...draft(), crop_assignment_id: unrelatedCropId, harvested_bushels: 777, harvest_date: '2026-07-10', actual_price_per_bu: 4.5 }); gateway.entries.set(unrelatedCropId, structuredClone(unrelatedBefore)); const repository = live(gateway); const saved = await repository.saveHarvest(draft()); const stored = gateway.entries.get(cropId)!; assert(saved.id === cropId && saved.crop_assignment_id === cropId && saved.farm_id === farm && saved.harvested_bushels === 1280.13 && saved.harvest_date === '2026-07-11' && saved.actual_price_per_bu === 4.567891 && saved.updated_at === stamp && gateway.saves.length === 1, 'A harvest save must return the complete SQL-canonical actual harvest record.'); assert(gateway.saves[0].entry.harvested_bushels === 1280.13 && gateway.saves[0].entry.actual_price_per_bu === 4.567891, 'Outgoing writes must use the same SQL decimal scales as the canonical echo.'); assert(stored.id === cropId && stored.farm_id === farm && stored.field_id === uid(5) && stored.crop_year === 2026 && stored.commodity_id === uid(6) && stored.planting_sequence === 1 && stored.planted_acres === 10 && stored.variety === null && stored.planting_date === '2026-04-20' && stored.expected_yield_per_acre === 190 && stored.expected_price_per_bu === 4.25 && stored.notes === null && stored.created_at === stamp && stored.updated_at === stamp, 'Harvest writes must preserve every related crop identity, acreage, commodity, and expected-plan field outside the three allowed actual harvest fields.'); assert(JSON.stringify(gateway.entries.get(unrelatedCropId)) === JSON.stringify(unrelatedBefore), 'A harvest write must leave every field on an unrelated crop unchanged.')
  ...
  // Group 10: receipt lifecycle is keyed by crop assignment, not operation ID, across connected, offline, replay, lost response, and blocked replay paths.
  const receiptGateway = new FakeGateway(); let releaseConnected!: () => void; const connectedGate = new Promise<void>((resolve) => { releaseConnected = resolve }); receiptGateway.beforeSave = () => connectedGate
  const connected = new QueuedHarvestRepository(live(receiptGateway), { getContext: async () => ({ userId: actor, farmId: farm }), projectRef: 'receipt-connected', storage: memory(), createId: (() => { let id = 800; return () => uid(id++) })(), clock: () => stamp, isOffline: () => false })
  const connectedSaving = connected.saveHarvest(draft()); await Promise.resolve(); assert(getSaveReceipt(cropId) === 'saving', 'A delayed connected harvest must publish Saving under its crop assignment ID before the response.'); releaseConnected(); await connectedSaving; assert(getSaveReceipt(cropId) === 'saved', 'A connected harvest must publish Saved after the durable response.')

  const offlineStore = memory(); let receiptOffline = true; const offlineGateway = new FakeGateway(); const offlineRepository = new QueuedHarvestRepository(live(offlineGateway), { getContext: async () => ({ userId: actor, farmId: farm }), projectRef: 'receipt-offline', storage: offlineStore, createId: (() => { let id = 810; return () => uid(id++) })(), clock: () => stamp, isOffline: () => receiptOffline })
  const offlineDraft = { ...draft(), expected_updated_at: stamp }; const offlinePending = await offlineRepository.saveHarvest(offlineDraft); const offlineQueue = new HarvestWriteQueue(offlineStore, harvestWriteQueueKey('receipt-offline', actor, farm)); const offlineEntry = offlineQueue.read().entries[0]
  assert(offlinePending.pending && getSaveReceipt(cropId) === 'queued offline' && Object.keys(offlineEntry.draft).length === 5 && offlineEntry.draft.crop_assignment_id === offlineDraft.crop_assignment_id && offlineEntry.draft.harvested_bushels === offlineDraft.harvested_bushels && offlineEntry.draft.harvest_date === offlineDraft.harvest_date && offlineEntry.draft.actual_price_per_bu === offlineDraft.actual_price_per_bu && offlineEntry.draft.expected_updated_at === stamp, 'Offline harvest must retain the exact four-field draft plus its expected version and publish Queued offline.')
  const replayStore = memory(); let replayOffline = true; const receiptReplayGateway = new FakeGateway(); const receiptReplayRepository = new QueuedHarvestRepository(live(receiptReplayGateway), { getContext: async () => ({ userId: actor, farmId: farm }), projectRef: 'receipt-replay', storage: replayStore, createId: (() => { let id = 815; return () => uid(id++) })(), clock: () => stamp, isOffline: () => replayOffline }); await receiptReplayRepository.saveHarvest(draft()); await new Promise<void>((resolve) => setTimeout(resolve, 0)); replayOffline = false; const replayQueue = new HarvestWriteQueue(replayStore, harvestWriteQueueKey('receipt-replay', actor, farm)); await receiptReplayRepository.inspectAndReplay(); assert(replayQueue.read().entries.length === 0 && getSaveReceipt(cropId) === 'saved', 'Replay must clear the confirmed queue entry and publish Saved for the same crop assignment.')

  const lostStore = memory(); let lostOffline = false; const lostGateway = new FakeGateway(); lostGateway.throwAfterPersist = true; lostGateway.afterPersist = () => { lostOffline = true }
  const lostRepository = new QueuedHarvestRepository(live(lostGateway), { getContext: async () => ({ userId: actor, farmId: farm }), projectRef: 'receipt-lost', storage: lostStore, createId: (() => { let id = 820; return () => uid(id++) })(), clock: () => stamp, isOffline: () => lostOffline })
  await lostRepository.saveHarvest(draft()); const lostQueue = new HarvestWriteQueue(lostStore, harvestWriteQueueKey('receipt-lost', actor, farm)); assert(lostQueue.read().entries.length === 1 && lostGateway.saves.length === 1 && lostGateway.persists === 1, 'A lost response must retain exactly one durable queued operation after its first attempt.')
  lostOffline = false; lostGateway.throwAfterPersist = false; await lostRepository.inspectAndReplay(); const lostReplayAttempts = Number(lostGateway.saves.length); const lostPersists = Number(lostGateway.persists); const lostOperationIds = lostGateway.saves.map((call) => call.operationId); const lostQueueLength = lostQueue.read().entries.length; const lostReceipt = getSaveReceipt(cropId); assert(lostReplayAttempts === 2 && lostPersists === 1 && new Set(lostOperationIds).size === 1 && lostQueueLength === 0 && lostReceipt === 'saved', `A lost response must replay the same operation exactly once, avoid a second persist, clear the queue, and publish Saved (attempts=${lostReplayAttempts}; persists=${lostPersists}; operationIds=${lostOperationIds.join(',')}; queueLength=${lostQueueLength}; receipt=${lostReceipt ?? 'none'}).`)

  const terminalStore = memory(); let terminalOffline = true; const terminalGateway = new FakeGateway(); const terminalRepository = new QueuedHarvestRepository(live(terminalGateway), { getContext: async () => ({ userId: actor, farmId: farm }), projectRef: 'receipt-terminal', storage: terminalStore, createId: (() => { let id = 830; return () => uid(id++) })(), clock: () => stamp, isOffline: () => terminalOffline })
  await terminalRepository.saveHarvest(draft()); const terminalQueue = new HarvestWriteQueue(terminalStore, harvestWriteQueueKey('receipt-terminal', actor, farm)); const retained = structuredClone(terminalQueue.read().entries[0]); const remoteBefore = structuredClone([...terminalGateway.entries.entries()]); terminalOffline = false; terminalGateway.terminalBeforePersist = new Error('validation failed before persist'); await terminalRepository.inspectAndReplay()
  assert(getSaveReceipt(cropId) === 'needs attention' && JSON.stringify(terminalQueue.read().entries[0]) === JSON.stringify(retained) && JSON.stringify([...terminalGateway.entries.entries()]) === JSON.stringify(remoteBefore), 'A terminal replay failure before persist must retain the exact existing queue entry, leave remote harvest unchanged, and publish Needs attention.')
  console.log('SupabaseHarvestRepository regression passed (10 coverage groups)')
}
void run()
