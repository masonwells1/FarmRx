...
async function run() {
  ...
  lostOffline = false; lostGateway.throwAfterPersist = false; await lostRepository.inspectAndReplay(); const lostReplayAttempts = Number(lostGateway.saves.length); const lostPersists = Number(lostGateway.persists); assert(lostReplayAttempts === 2 && lostPersists === 1 && new Set(lostGateway.saves.map((call) => call.operationId)).size === 1 && lostQueue.read().entries.length === 0 && getSaveReceipt(cropId) === 'saved', 'A lost response must replay the same operation exactly once, avoid a second persist, clear the queue, and publish Saved.')
