...
const bid = (id: string, elevator: string, bid_date: string, cash_price: number | null, basis: number, notes: string | null = null): CashBid => ({ id, farm_id: farmA, elevator, commodity_id: 'corn_yellow', bid_date, basis, cash_price, delivery_start: null, delivery_end: null, notes, feed_source: null, feed_report_id: null, feed_geography: null, created_at: now, updated_at: now })
...
const grainWorkspace: GrainWorkspace = {
  ...
  grain_contract_deliveries: [], grain_loads: [], insurance_units: [], grain_bins: [], bin_inventory: [], bin_transactions: [], usda_market_reports: [], usda_report_dates: [], marketing_alert_rules: [], firm_offers: [], grain_alert_settings: null, grain_sale_limits: [], grain_carry_settings: null, grain_carry_grids: [],
...
const columnOnlyFeed = todayGrainLine({ profile: owner, grain: { ...grainWorkspace, cash_bids: [...grainWorkspace.cash_bids, { ...bid('00000000-0000-4000-8000-000000000e71', 'Ames', '2026-07-16', 4.9, -0.05), feed_source: 'usda_mars', feed_report_id: '3101', feed_geography: 'IL' }] }, today })
assert.equal(columnOnlyFeed?.detail.split(' · ')[1], 'Cargill Olney $4.12, up 5¢ since Jul 10', 'A feed row recognized only by its provenance column never becomes the local bid (GL-1).')
