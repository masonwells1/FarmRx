...
export class QueuedProfitabilityRepository implements ProfitabilityRepository {
  ...
  private async write(entry: ProfitabilityQueueEntryV1, operationContext: FarmOperationContext, memoryGuard: WorkspaceMemoryGuard) {
    await this.verifyDirect(entry, operationContext, memoryGuard)
    switch (entry.kind) {
  ...
  private async verifyDirect(context: Context, operationContext: FarmOperationContext, memoryGuard: WorkspaceMemoryGuard) { verifyFarmOperationContext(this.dependencies.storage, operationContext, memoryGuard.fence); this.memoryScope.verify(this.dependencies.storage, memoryGuard); const current = await this.dependencies.getContext(); if (current.userId !== context.userId || current.farmId !== context.farmId || operationContext.userId !== context.userId || operationContext.farmId !== context.farmId) throw new Error(blocked); verifyFarmOperationContext(this.dependencies.storage, operationContext, memoryGuard.fence); this.memoryScope.verify(this.dependencies.storage, memoryGuard) }
    ...
    try {
      await this.locked(queue, async (verify) => {
        ...
        while (envelope.entries.length) {
          ...
          try { const saved = await this.write(this.rebase(head, versions), memoryGuard.fence, memoryGuard); const key = this.mutableKey(head); if (key && saved && typeof saved === 'object' && typeof (saved as { updated_at?: unknown }).updated_at === 'string') versions.set(key, (saved as { updated_at: string }).updated_at); verify(); await this.verifyOperation(head, memoryGuard.fence, memoryGuard); envelope = queue.removeConfirmedHead(head.operationId) }
          catch (error) {
            await this.verifyOperation(head, memoryGuard.fence, memoryGuard)
