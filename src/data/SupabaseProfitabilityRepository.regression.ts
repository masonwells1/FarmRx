...
async function run() {
  ...
  ;(gateway.state.bundle.cost_lines as Array<Record<string, unknown>>).push({ id: uid(718), farm_id: gateway.state.scope.farm_id, budget_id: savedBudget.id, category: 'seed', label: 'Seed (U of I)', amount_per_acre: '118.0000', source_kind: 'manual', source_record_id: null, sort_order: 3, notes: null, equipment_period_start: null, equipment_period_end: null, equipment_total_source_amount: null, equipment_allocation_acres: null, equipment_included_row_count: null, equipment_excluded_null_cost_count: null, equipment_captured_at: null, university_default_amount: '118.0000', created_at: stamp, updated_at: stamp })
  ...
  assert(copyCall.costLines.length === 3 && copyCall.costLines.every((line, index) => line.source_kind !== 'equipment' && line.sort_order === index), 'copyBudget must omit dated equipment snapshots instead of disguising them as manual costs.')
  assert(copyCall.costLines.every((line) => line.university_default_amount === null), 'copyBudget must not carry the U of I default badge onto copied lines.')
