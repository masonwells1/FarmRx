...
const tableMissing = (error: { code?: string } | null | undefined) => error?.code === '42P01' || error?.code === 'PGRST205'
...
export class SupabaseGrainDataGateway implements GrainDataGateway {
  async loadWorkspace(farmId: string): Promise<GrainRowBundle> {
    ...
    // The slice-3 settings tables ship in one migration; any one of them missing means the release is not applied yet.
    const persistedSettings = !(tableMissing(grain_sale_limits.error) || tableMissing(grain_carry_settings.error) || tableMissing(grain_carry_grids.error))
    if (persistedSettings && grain_carry_settings.error) throw grain_carry_settings.error
    return { production_estimates: rows(production_estimates.data, production_estimates.error), grain_contracts: rows(grain_contracts.data, grain_contracts.error), grain_contract_deliveries: deliveriesUnavailable ? [] : rows(grain_contract_deliveries.data, grain_contract_deliveries.error), marketing_plan_targets: rows(marketing_plan_targets.data, marketing_plan_targets.error), insurance_units: rows(insurance_units.data, insurance_units.error), grain_bins: rows(grain_bins.data, grain_bins.error), bin_inventory: rows(bin_inventory.data, bin_inventory.error), bin_transactions: rows(bin_transactions.data, bin_transactions.error), cash_bids: rows(cash_bids.data, cash_bids.error), usda_report_dates: rows(usda_report_dates.data, usda_report_dates.error), marketing_alert_rules: rows(marketing_alert_rules.data, marketing_alert_rules.error), firm_offers: rows(firm_offers.data, firm_offers.error), grain_alert_settings: grain_alert_settings.data, grain_sale_limits: persistedSettings ? rows(grain_sale_limits.data, grain_sale_limits.error) : [], grain_carry_settings: persistedSettings ? grain_carry_settings.data : null, grain_carry_grids: persistedSettings ? rows(grain_carry_grids.data, grain_carry_grids.error) : [], capabilities: { bin_movements: post0033, contract_price_finalization: post0033, contract_deliveries: post0033, persisted_settings: persistedSettings } }
  }
