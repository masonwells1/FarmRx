...
import type { BinTransaction, CashBid, FirmOffer, GrainAlertSettings, GrainBin, GrainCarryGrid, GrainCarrySettings, GrainContract, GrainContractCorrection, GrainContractDelivery, GrainSaleLimit, MarketingAlertRule, ProductionEstimate } from './grain'
...
export class SupabaseGrainDataGateway implements GrainDataGateway {
  ...
  async editContractRpc(farmId: string, contractId: string, reason: string, changes: GrainContractCorrection, context: FarmOperationContext) { const { data, error } = await bindFarmOperationRequest(supabase.rpc('edit_grain_contract', { p_farm_id: farmId, p_contract_id: contractId, p_reason: reason, p_changes: changes }), context); if (error) throw error; return row(data, null) }
  async deleteContractRpc(farmId: string, contractId: string, reason: string, context: FarmOperationContext) { const { data, error } = await bindFarmOperationRequest(supabase.rpc('delete_grain_contract', { p_farm_id: farmId, p_contract_id: contractId, p_reason: reason }), context); if (error) throw error; return data }
