...
// LD-5: crop_year travels with a manual movement. append_bin_movement has read it since LD-2, but
// this list never carried it, so every hand-entered movement landed in the unknown-year bucket.
function binTransactionColumns(value: BinTransaction) { const { id, farm_id, grain_bin_id, direction, bushels, commodity_id, crop_year, occurred_on, note, source_kind } = value; return { id, farm_id, grain_bin_id, direction, bushels, commodity_id, crop_year, occurred_on, note, source_kind } }
