...
export class QueuedHarvestRepository implements HarvestRepository {
  constructor(private readonly live: SupabaseHarvestRepository, private readonly d: { getContext: () => Promise<Context>; projectRef: string; storage: StorageLike; createId: () => string; clock: () => string; isOffline: () => boolean }) { setModuleSyncRetryAction('harvest', () => { void this.inspectAndReplay() }) }
