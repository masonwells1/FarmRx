...
export class QueuedHarvestRepository implements HarvestRepository {
  ...
  async saveHarvest(draft: HarvestDraft) {
    ...
    const receiptId = draft.crop_assignment_id; setSaveReceipt(receiptId, 'saving')
    return this.locked(queue.key, async (verify) => { const verifyOperation = async () => { verify(); await verifyQueuedOperationContext(this.d, operationContext, entry) }; await verifyOperation(); if (this.d.isOffline() || queue.read().entries.length) { await verifyOperation(); const next = queue.append(entry); setModuleSyncStatus('harvest', { kind: 'pending', pending: next.entries.length }); setSaveReceipt(receiptId, 'queued offline'); launchReplayInBackground(() => this.inspectAndReplay()); return pendingRecord(entry, context) }
      try { const result = await this.send(entry, operationContext); await verifyOperation(); setModuleSyncStatus('harvest', { kind: 'synced', pending: 0 }); setSaveReceipt(receiptId, 'saved'); return result } catch (error) { await verifyQueuedOperationContext(this.d, operationContext, entry); if (!isTransportFailure(error, this.d.isOffline())) { setSaveReceipt(receiptId, 'needs attention'); throw error }; await verifyOperation(); const next = queue.append(entry); setModuleSyncStatus('harvest', { kind: 'pending', pending: next.entries.length }); setSaveReceipt(receiptId, 'queued offline'); launchReplayInBackground(() => this.inspectAndReplay()); return pendingRecord(entry, context) } })
  }
