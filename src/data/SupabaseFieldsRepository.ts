...
export function mapFarm(value: unknown): Farm { const row = strictRow(value, 'Farm'); return { id: text(row.id, 'Farm ID'), name: text(row.name, 'Farm name'), share_with_rep: bool(row.share_with_rep, 'Farm sharing setting'), time_zone: Object.hasOwn(value as object, 'time_zone') && typeof (value as Record<string, unknown>).time_zone === 'string' ? (value as Record<string, unknown>).time_zone as string : null, created_by: text(row.created_by, 'Farm creator'), created_at: iso(row.created_at, 'Farm created date'), updated_at: iso(row.updated_at, 'Farm updated date') } }
