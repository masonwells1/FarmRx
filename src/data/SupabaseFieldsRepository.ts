...
export function mapFarm(value: unknown): Farm { const row = strictRow(value, 'Farm'); return { id: text(row.id, 'Farm ID'), name: text(row.name, 'Farm name'), share_with_rep: bool(row.share_with_rep, 'Farm sharing setting'), time_zone: Object.hasOwn(value as object, 'time_zone') && typeof (value as Record<string, unknown>).time_zone === 'string' ? (value as Record<string, unknown>).time_zone as string : null, market_region: Object.hasOwn(value as object, 'market_region') && typeof (value as Record<string, unknown>).market_region === 'string' ? (value as Record<string, unknown>).market_region as string : null, created_by: text(row.created_by, 'Farm creator'), created_at: iso(row.created_at, 'Farm created date'), updated_at: iso(row.updated_at, 'Farm updated date') } }
...
/** GL-2's marketing-year columns are read from the RAW record, not through `strictRow`. That proxy
 * fails closed on any column the row does not carry, which is the right rule for a column the app
 * depends on — and the wrong one here: these two are absent until the GL-2 migration is applied, and a
 * farm on the previous schema must still load its fields. Absent means "fall back to the crop family's
 * USDA default" (src/data/marketingYear.ts), not "this workspace is broken". */
function optionalSmallInt(raw: Record<string, unknown>, key: string): number | null {
  if (!Object.hasOwn(raw, key)) return null
  const value = raw[key]
  return typeof value === 'number' && Number.isInteger(value) ? value : null
}
export function mapCommodity(value: unknown): Commodity { const raw = record(value, 'Commodity'); const row = strictRow(value, 'Commodity'); const crop_family = text(row.crop_family, 'Commodity family'); if (!cropFamilies.has(crop_family as Commodity['crop_family'])) fail('Commodity family is malformed.'); const traits = record(row.traits, 'Commodity traits'); const marketingMonth = optionalSmallInt(raw, 'marketing_year_start_month'); const marketingDay = optionalSmallInt(raw, 'marketing_year_start_day'); return { id: text(row.id, 'Commodity ID'), name: text(row.name, 'Commodity name'), crop_family: crop_family as Commodity['crop_family'], marketing_year_start_month: marketingMonth, marketing_year_start_day: marketingDay, traits, is_active: bool(row.is_active, 'Commodity active setting'), created_at: iso(row.created_at, 'Commodity created date'), updated_at: iso(row.updated_at, 'Commodity updated date') } }
