...
assert.match(inventoryModule, /useLayoutEffect\(\(\) => \{[\s\S]*const tabListRect = tabList\.getBoundingClientRect\(\)[\s\S]*const activeTabRect = activeTab\.getBoundingClientRect\(\)[\s\S]*activeTabRect\.left < tabListRect\.left[\s\S]*tabList\.scrollLeft -= tabListRect\.left - activeTabRect\.left[\s\S]*activeTabRect\.right > tabListRect\.right[\s\S]*tabList\.scrollLeft \+= activeTabRect\.right - tabListRect\.right[\s\S]*\}, \[view, workspaceReady\]\)/, 'A selected Inventory tab must move only the tab scroller to the nearest hidden edge when the view changes or first becomes ready.')
