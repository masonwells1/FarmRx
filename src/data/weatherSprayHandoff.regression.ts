...
assert.match(inventoryModule, /return <SprayFormContents key=\{location\.key\}/, 'Each route entry must remount Spray so a same-path state change cannot retain old values.')
assert.match(inventoryModule, /props\.workspace\.fields\.fields\.some\(\(field\) => field\.id === routePrefill\.fieldId\)/, 'A prefilled field must exist in the loaded farm workspace.')
...
assert.match(inventoryModule, /onPrefillConsumed=\{\(\) => setSprayHandoff\(null\)\}/, 'A successful save must consume stale prefill state.')
