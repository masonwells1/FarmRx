...
assert.match(inventoryModule, /const manualSprayRecord = isManualSprayRecordIntent\(location\.state\)/, 'Inventory must derive one exact boolean from the payload-free manual intent.')
assert.match(inventoryModule, /useState<'shelf' \| 'receive' \| 'adjust' \| 'spray' \| 'compliance'>\(\(\) => manualSprayRecord \? 'spray' : 'shelf'\)/, 'The payload-free intent boolean must open the Spray tab.')
assert.match(inventoryModule, /useLayoutEffect\(\(\) => \{\s*if \(!manualSprayRecord\) return\s*window\.scrollTo\(\{ top: 0, left: 0, behavior: 'auto' \}\)\s*\}, \[location\.key, manualSprayRecord\]\)/, 'Each distinct manual handoff route entry must reset the document to the top with an idempotent absolute scroll.')
...
const tabAlignmentEffect = inventoryModule.match(/useLayoutEffect\(\(\) => \{\r?\n    const tabList = tabListRef\.current[\s\S]*?\r?\n  \}, \[view, workspaceReady\]\)/)?.[0]
assert.ok(tabAlignmentEffect, 'The bounded Inventory tab-alignment effect must remain independently identifiable across LF and CRLF checkouts.')
assert.doesNotMatch(tabAlignmentEffect, /window\.scrollTo|scrollIntoView|document\.(?:body|documentElement)\.scroll/, 'The generic tab-alignment effect must never scroll the document.')
...
assert.equal(inventoryModule.match(/window\.scrollTo\(/g)?.length, 1, 'Inventory may contain only the one exact manual-handoff document scroll.')
const inventoryWithoutManualTopReset = inventoryModule.replace("window.scrollTo({ top: 0, left: 0, behavior: 'auto' })", '')
assert.doesNotMatch(inventoryWithoutManualTopReset, /scrollIntoView|scrollTo\(|window\.scroll|document\.(?:body|documentElement)\.scroll/, 'Inventory must not retain any broad element or document scrolling outside the exact manual-handoff top reset.')
