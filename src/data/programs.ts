 ...
 * or reassigning its program cancels every planned pass of the assignment; taking program updates cancels a planned pass the
 * template no longer has, moves one whose template date changed, or leaves one unscheduled when the template date went away
 * (the server also closes or moves their generated tasks when the entry lands); a reschedule moves its due date. Keys are
 * lower-case pass ids. */
export type PendingPassOutcome = { kind: 'applied' | 'skipped' | 'cancelled' | 'unscheduled' } | { kind: 'rescheduled'; dueOn: string }
...
export interface ProgramsRepository { getData(includeArchived?: boolean): Promise<ProgramsData>; saveProgram(draft: ProgramDraft): Promise<Program>; saveProgramPass(programId: string, pass: ProgramPassDraft, products: ProgramProductDraft[], placeAfterPassId: string | null): Promise<ProgramPass>; reorderProgramPasses(programId: string, orderedPassIds: string[]): Promise<string[]>; deleteProgramPass(programId: string, passId: string): Promise<void>; deleteProgram(programId: string): Promise<Program>; assignProgram(programId: string, cropAssignmentIds: string[]): Promise<ProgramAssignment[]>; refreshProgramAssignment(assignmentId: string): Promise<ProgramAssignment>; reassignProgramAssignment(assignmentId: string, newProgramId: string, reason: string): Promise<ProgramAssignment>; rescheduleProgramPass(assignedPassId: string, dueOn: string, timingLabel: string | null): Promise<AssignedProgramPass>; markProgramPassApplied(assignedPassId: string, appliedOn: string, appliedAcres: number, actualProducts: ActualProgramProduct[], applicationLink?: ProgramApplicationLink): Promise<AssignedProgramPass>; skipProgramPass(assignedPassId: string, skippedOn: string, reason: string): Promise<AssignedProgramPass>; unassignProgram(assignmentId: string, reason: string): Promise<void>; /** Pure read for projections such as Today: the outcomes this device has queued for passes but not yet synced. Nothing is fetched, replayed or written. */ getPendingPassOutcomes?(context: FarmOperationContext): Promise<ReadonlyMap<string, PendingPassOutcome>> }
