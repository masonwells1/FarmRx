...
async function run() {
  ...
  const serviceWorker = readFileSync(new URL('../sw.ts', import.meta.url), 'utf8'); assert(serviceWorker.includes("new NavigationRoute(createHandlerBoundToURL('/index.html'), { denylist: [/^\\/update-password(?:[/?]|$)/] })") && serviceWorker.includes("url.search === '?recovery_entry=1'") && serviceWorker.includes('await client.navigate(client.url)') && serviceWorker.includes('self.skipWaiting()') && serviceWorker.includes('clientsClaim()'), 'The service worker must serve cached index.html for ordinary offline navigation, bypass current recovery redirects, and reload an unconsumed legacy recovery entry after activation.'); assert(serviceWorker.includes('plainObject(parsed) ? parsed : {}') && serviceWorker.includes('notificationText(payload.title, 160') && serviceWorker.includes('notificationText(payload.body, 500'), 'Malformed push payloads must fall back safely and cap notification text.')
