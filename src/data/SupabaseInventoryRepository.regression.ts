...
async function run() {
  ...
  const app = applicationInput(uid(30)); const confirmedApplication = await live.saveApplication(app)
  assert(confirmedApplication.kind === 'confirmed' && confirmedApplication.applicationId === app.id && gateway.calls.application === 1 && gateway.state.application_products.length === 2, 'A confirmed application must return a matching confirmed disposition after its canonical server echo.')
  ...
  const offlineApplicationStore = memory(); const offlineApplicationGateway = new FakeGateway(); const offlineApplication = applicationInput(uid(53)); const offlineApplications = new QueuedInventoryRepository(repo(offlineApplicationGateway), { getContext: async () => ({ userId: actor, farmId: farm }), projectRef: 'queued-application', storage: offlineApplicationStore, createId: () => uid(54), clock: () => micro, isOffline: () => true })
  const queuedApplication = await offlineApplications.saveApplication(offlineApplication); const queuedApplicationKey = inventoryWriteQueueKey('queued-application', actor, farm)
  const queuedApplicationEntry = new InventoryWriteQueue(offlineApplicationStore, queuedApplicationKey).read().entries[0]
  assert(queuedApplication.kind === 'queued' && queuedApplication.applicationId === offlineApplication.id && queuedApplication.operationId === queuedApplicationEntry?.operationId && queuedApplicationEntry?.kind === 'saveApplicationBundle' && offlineApplicationGateway.calls.application === 0 && getSaveReceipt(offlineApplication.id) === 'queued offline', 'An offline application must return queued custody, keep its exact operation identity, and never claim server confirmation.')
  const attentionApplicationStore = memory(); const attentionApplicationGateway = new FakeGateway(); attentionApplicationGateway.mutate.application = (reply) => ({ ...reply, products: reply.products.slice(0, 1) }); const attentionApplication = applicationInput(uid(55)); const attentionApplications = new QueuedInventoryRepository(repo(attentionApplicationGateway), { getContext: async () => ({ userId: actor, farmId: farm }), projectRef: 'attention-application', storage: attentionApplicationStore, createId: () => uid(56), clock: () => micro, isOffline: () => false })
  const attentionApplicationResult = await attentionApplications.saveApplication(attentionApplication); const attentionApplicationKey = inventoryWriteQueueKey('attention-application', actor, farm)
  assert(attentionApplicationResult.kind === 'needs-attention' && attentionApplicationResult.applicationId === attentionApplication.id && new InventoryWriteQueue(attentionApplicationStore, attentionApplicationKey).read().entries.length === 0 && readNeedsAttention(attentionApplicationStore, attentionApplicationKey).length === 1 && getSaveReceipt(attentionApplication.id) === 'needs attention', 'An unconfirmed application response must return and durably park Needs attention instead of being presented as saved.')
  ...
  // Program application rows are tenant-checked and are always unmatched:
  // confirmed Program Inventory history belongs to the no-record Programs view.
  const unmatchedFacts = { inventory_matched: false, inventory_product_id: null, quantity_in_inventory_unit: null, inventory_product_name_snapshot: null, inventory_unit_snapshot: null, inventory_operation_id: null, inventory_confirmed_by: null, inventory_confirmed_at: null }
  const applicationRow = (idValue: string, status: 'draft' | 'completed') => ({ id: idValue, farm_id: farm, field_id: fieldsFor(farm).crop_assignments[0].field_id, crop_assignment_id: fieldsFor(farm).crop_assignments[0].id, status, application_date: '2026-07-12', start_time: null, applied_acres: 10, target_pest: null, applicator_name_snapshot: null, applicator_license_number_snapshot: null, wind_speed_mph: null, wind_direction: null, temperature_f: null, relative_humidity_pct: null, completed_at: status === 'completed' ? micro : null, void_reason: null, corrects_application_id: null, created_at: micro, created_by: actor })
  const programLinkGateway = new FakeGateway(); const programLinkRepo = repo(programLinkGateway); await programLinkRepo.saveApplication(applicationInput(uid(90))); programLinkGateway.state.program_application_products = [{ farm_id: farm, application_record_id: uid(90), assigned_pass_id: uid(91), assignment_id: uid(92), assigned_product_id: uid(93), sequence: 1, actual_product_name: 'Free typed Program line', actual_rate_text: '1', actual_unit_text: 'qt/ac', actual_cost_per_acre: 12, ...unmatchedFacts }]
  const completedProgramWorkspace = await programLinkRepo.getWorkspace(); const productlessDraftGateway = new FakeGateway(); productlessDraftGateway.state.applications = [applicationRow(uid(94), 'draft')]; productlessDraftGateway.state.program_application_products = [{ farm_id: farm, application_record_id: uid(94), assigned_pass_id: uid(95), assignment_id: uid(96), assigned_product_id: uid(97), sequence: 1, actual_product_name: 'Draft free typed Program line', actual_rate_text: '2', actual_unit_text: 'qt/ac', actual_cost_per_acre: null, ...unmatchedFacts }]
  const productlessDraftWorkspace = await repo(productlessDraftGateway).getWorkspace(); const cards = [...deriveProgramApplicationRecordCards(completedProgramWorkspace), ...deriveProgramApplicationRecordCards(productlessDraftWorkspace)]; const completedCard = cards.find((card) => card.record.id === uid(90)); const draftCard = cards.find((card) => card.record.id === uid(94)); assert(completedCard?.statusLabel === 'Completed record' && completedCard.inventoryLines.length === 2 && completedCard.programLines[0]?.actual_product_name === 'Free typed Program line' && draftCard?.statusLabel === 'Draft / un-posted' && draftCard.inventoryLines.length === 0 && draftCard.programLines[0]?.actual_product_name === 'Draft free typed Program line', 'Completed links and zero-product drafts must derive safe, separately labelled unmatched Program lines.')
  const impossibleMatchedRow = { farm_id: farm, application_record_id: uid(98), assigned_pass_id: uid(99), assignment_id: uid(100), assigned_product_id: uid(101), sequence: 1, actual_product_name: 'Atrazine', actual_rate_text: '1', actual_unit_text: 'qt/ac', actual_cost_per_acre: 12, inventory_matched: true, inventory_product_id: uid(501), quantity_in_inventory_unit: '4.25', inventory_product_name_snapshot: 'Atrazine', inventory_unit_snapshot: 'gal', inventory_operation_id: uid(102), inventory_confirmed_by: actor, inventory_confirmed_at: micro }
  const impossibleMatchedGateway = new FakeGateway(); impossibleMatchedGateway.state.applications = [applicationRow(uid(98), 'draft')]; impossibleMatchedGateway.state.program_application_products = [impossibleMatchedRow]
  await rejects(() => repo(impossibleMatchedGateway).getWorkspace(), 'An application-linked Program projection must fail closed on inventory_matched=true.')
  for (const forged of [{ ...impossibleMatchedRow, inventory_matched: false }, { ...impossibleMatchedRow, inventory_product_id: null }]) { const gateway = new FakeGateway(); gateway.state.applications = [applicationRow(uid(98), 'draft')]; gateway.state.program_application_products = [forged]; await rejects(() => repo(gateway).getWorkspace(), 'Application-linked Program rows must reject hidden or partial Inventory match facts.') }
  assert(completedProgramWorkspace.program_application_products[0]?.inventory_matched === false, 'A normal application-linked Program row must remain readable as explicitly unmatched.')
  const foreignProgramRow = new FakeGateway(); foreignProgramRow.state.applications = [applicationRow(uid(94), 'draft')]; foreignProgramRow.state.program_application_products = [{ farm_id: uid(998), application_record_id: uid(94), assigned_pass_id: uid(95), assignment_id: uid(96), assigned_product_id: uid(97), sequence: 1, actual_product_name: 'Foreign line', actual_rate_text: '1', actual_unit_text: 'qt/ac', actual_cost_per_acre: null, ...unmatchedFacts }]; await rejects(() => repo(foreignProgramRow).getWorkspace(), 'A foreign-farm Program application row must fail closed.')
  console.log('SupabaseInventoryRepository regression passed (10 coverage groups)')
}
void run()
