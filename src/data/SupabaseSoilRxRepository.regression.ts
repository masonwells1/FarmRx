...
assert.match(migration, /This function never deletes an object[\s\S]*failed first save has no test yet/, 'Absence verification must permit the no-row terminal cleanup case without granting object deletion.')
assert.doesNotMatch(migration, /soil report path is not owned by the current farm test/, 'Absence verification must not strand a never-uploaded attachment when its provisional test row was never created.')
