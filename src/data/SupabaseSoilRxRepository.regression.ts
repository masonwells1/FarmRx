...
assert.match(migration, /grant\s+execute\s+on\s+function\s+public\.verify_soil_test_absent\(uuid,\s*uuid\),\s*public\.verify_soil_report_objects_absent\(uuid,\s*text\[\]\),\s*public\.verify_soil_report_cleanup_terminal_absence\(uuid,\s*uuid,\s*uuid,\s*text\[\]\)\s+to\s+authenticated;/i, 'Soil absence verifiers must be executable only by authenticated callers.')
assert.match(migration, /revoke\s+all\s+on\s+function\s+public\.verify_soil_test_absent\(uuid,\s*uuid\),\s*public\.verify_soil_report_objects_absent\(uuid,\s*text\[\]\),\s*public\.verify_soil_report_cleanup_terminal_absence\(uuid,\s*uuid,\s*uuid,\s*text\[\]\)\s+from\s+public,\s*anon,\s*authenticated,\s*service_role;/i, 'Soil absence verifiers must revoke execution from every role before granting authenticated access.')
