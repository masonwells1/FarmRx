...
type WriteState = { tests: Array<Record<string, unknown>>; attachments: Array<Record<string, unknown>>; writes: string[]; uploaded: Set<string>; failSoilSaves: number; failAttachmentMetadata: number; changeEpochAfterUpload: boolean; accessEpoch: number }
function writeState(): WriteState { return { tests: [], attachments: [], writes: [], uploaded: new Set(), failSoilSaves: 0, failAttachmentMetadata: 0, changeEpochAfterUpload: false, accessEpoch: 1 } }
...
async function mock(page: Page, readOnly = false, state?: WriteState, denied = false) {
  ...
  await page.route('https://*.supabase.co/**', async (route) => {
    ...
    if (state && request.method() === 'DELETE' && url.pathname === '/storage/v1/object/soil-test-reports') { const body = request.postDataJSON() as { prefixes?: string[] }; for (const path of body.prefixes ?? []) state.uploaded.delete(path); state.writes.push(`storage:remove:${(body.prefixes ?? []).join(',')}`); return json([]) }
...
test('Soil Rx saves text and attachments atomically without unrelated writes', async ({ page }) => {
  ...
  expect(state.writes.filter((entry) => /^(arrangements|crop_assignments|commodities|notifications):/.test(entry))).toEqual([])
  expect(unexpected).toEqual([])
})

test('Soil Rx offline replay surfaces guarded Retry and Dismiss', async ({ page, context }) => {
  const state = writeState(); const unexpected = await open(page, false, state)
