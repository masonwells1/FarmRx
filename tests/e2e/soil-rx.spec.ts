...
async function mock(page: Page, readOnly = false, state?: WriteState, denied = false) {
  ...
  await page.route('https://*.supabase.co/**', async (route) => {
    ...
    if (state && url.pathname.endsWith('/rpc/verify_soil_report_objects_absent')) { const body = request.postDataJSON() as { p_farm_id?: string; p_paths?: string[] }; const paths = body.p_paths ?? []; state.writes.push(`storage:verify-absent:${paths.join(',')}`); return json(body.p_farm_id === farm ? paths.filter((path) => !state.uploaded.has(path)).map((name) => ({ name })) : []) }
