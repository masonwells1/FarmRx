...
test('Soil Rx is phone-safe and shows newest history, partial measurements, privacy, and read-only locks', async ({ page }, testInfo) => {
  const ownerState = historyState(); ownerState.tests[0]!.ph = 6.4; ownerState.tests[0]!.organic_matter_pct = 3.1; ownerState.tests[0]!.cec_meq_100g = 14.2; ownerState.tests[0]!.base_saturation_calcium_pct = 68; const ownerProtectedBefore = protectedSnapshot(ownerState); const ownerUnexpected = await open(page, false, ownerState); await expect(page.getByText('Old Lab')).toBeVisible(); const missingMeasurement = page.locator('.soil-test-details').getByText('Not reported').first(); await expect(missingMeasurement).toBeVisible(); await expect(page.getByRole('region', { name: 'Understand this report' })).toContainText('not agronomic advice, target ranges, or a fertilizer recommendation'); await expect(page.getByRole('region', { name: 'Understand this report' })).toContainText('Lab result: 6.4'); await expect(page.getByRole('region', { name: 'Understand this report' })).toContainText('Calcium: 68'); await expect(page.locator('body')).toHaveJSProperty('scrollWidth', await page.locator('body').evaluate((node) => node.clientWidth));
