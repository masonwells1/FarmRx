...
test('an owner changes rep sharing only after confirmation and server proof', async ({ context, page }) => {
  ...
  const sharing = page.getByRole('switch', { name: 'Share my grain, financial, and Soil Rx data with my Crop RX rep' })
  await expect(sharing).toHaveAttribute('aria-checked', 'false')
  await sharing.click()
  const confirmation = page.getByRole('dialog')
  await expect(confirmation).toContainText('grain, financial, and Soil Rx data')
  expect(requests.patchRequests()).toBe(0)
  await confirmation.getByRole('button', { name: 'Turn sharing on' }).click()
  await expect(page.getByRole('switch', { name: 'Share my grain, financial, and Soil Rx data with my Crop RX rep' })).toHaveAttribute('aria-checked', 'true')
...
test('a failed response never flips the privacy control optimistically', async ({ context, page }) => {
  ...
  const sharing = page.getByRole('switch', { name: 'Share my grain, financial, and Soil Rx data with my Crop RX rep' })
  await sharing.click()
  await page.getByRole('dialog').getByRole('button', { name: 'Turn sharing on' }).click()
  await expect(sharing).toHaveAttribute('aria-checked', 'false')
...
test('a worker sees the privacy truth but cannot change it', async ({ context, page }) => {
  ...
  await page.goto('/privacy')
  await expect(page.getByRole('heading', { name: 'Farm settings' })).toBeVisible()
  // GL-1: the page holds two cards (rep sharing, market region); both tell a worker the same thing.
  await expect(page.getByText('Only a farm owner or manager can change this setting.')).toHaveCount(2)
