...
async function phoneNoOverflow(page: Page) { await page.setViewportSize({ width: 390, height: 844 }); const widths = await page.evaluate(() => ({ viewport: innerWidth, document: document.documentElement.scrollWidth, body: document.body.scrollWidth })); expect(widths.document).toBeLessThanOrEqual(widths.viewport); expect(widths.body).toBeLessThanOrEqual(widths.viewport) }
...
test('@maple-august-phone reads Done without writing', async ({ page }) => { const requests = phase(); const network = await fence(page, requests); await signIn(page); await phoneNoOverflow(page); await page.goto('/tasks'); await expect(page.locator('section.task-column').filter({ hasText: 'Done' }).getByText('Inspect Maple south gate')).toBeVisible(); expect(requests.observedTargetMutationPaths).toEqual([]); assertFence(requests, network.external) })
...
test('@maple-september-phone reads harvest without writing', async ({ page }) => { const requests = phase(); const network = await fence(page, requests); await signIn(page); await phoneNoOverflow(page); await page.goto('/harvest'); await expect(page.getByText(/192.5/)).toBeVisible(); assertFence(requests, network.external) })
...
test('@maple-november-out appends the independent bin-out movement', async ({ page }) => { const requests = phase({ rpcs: ['append_bin_movement'] }); const network = await fence(page, requests); await signIn(page); await page.goto('/grain/storage'); const bin = page.locator('article.bin-card').filter({ hasText: 'Maple North Bin' }); await bin.getByText(/Movement ledger/).click(); const form = bin.locator('form.movement-form'); await form.getByLabel('Direction').selectOption('out'); await form.getByLabel('Bushels').fill('5000'); await form.getByLabel('Date').fill('2027-11-10'); await form.getByLabel('Note').fill('Delivery to Synthetic Elevator'); await id(page, ids.outbound); await form.getByRole('button', { name: 'Add movement' }).click(); await expect(bin).toContainText('25,800 bu'); await replay(page, network.writes); assertFence(requests, network.external) })
test('@maple-november-delivery records the independent contract delivery', async ({ page }) => { const requests = phase({ rpcs: ['record_grain_contract_delivery'] }); const network = await fence(page, requests); await signIn(page); await page.goto('/grain/contracts'); const contract = page.locator('tr').filter({ hasText: 'MR-2027-001' }); await contract.getByLabel('Delivered bushels').fill('5000'); await id(page, ids.delivery); await contract.getByRole('button', { name: 'Record delivery' }).click(); await replay(page, network.writes); await expect(contract).toContainText('5,000.00 / 0.00 bu'); assertFence(requests, network.external) })
...
test('@maple-december-read-only reconciles the full year without a write', async ({ page }) => { const requests = phase(); const network = await fence(page, requests); await signIn(page); await page.goto('/fields'); await expect(page.getByText('Maple East 160')).toBeVisible(); await page.reload(); for (const path of ['/grain', '/grain/contracts', '/grain/storage', '/inventory', '/programs', '/harvest', '/scouting', '/tasks']) { await page.goto(path); await page.reload() }; await expect(page.getByText('Inspect Maple south gate')).toBeVisible(); await assertDecemberClean(page); assertFence(requests, network.external) })
test('@maple-december-phone reads the year closeout without writing', async ({ page }) => { const requests = phase(); const network = await fence(page, requests); await signIn(page); await phoneNoOverflow(page); await page.goto('/grain'); await expect(page.locator('.grain-reconciliation')).toContainText('30,800'); await page.goto('/tasks'); await expect(page.getByText('Inspect Maple south gate')).toBeVisible(); await assertDecemberClean(page); assertFence(requests, network.external) })
async function decemberRoute(page: Page, path: string, visible: string | RegExp, tab?: string) { const requests = phase(); const network = await fence(page, requests); await signIn(page); await page.goto(path); if (tab) await page.getByRole('button', { name: tab }).click(); await page.reload(); if (tab) await page.getByRole('button', { name: tab }).click(); await expect(page.getByText(visible)).toBeVisible(); await assertDecemberClean(page); assertFence(requests, network.external) }
test('@maple-december-fields', async ({ page }) => { await decemberRoute(page, '/fields', 'Maple East 160') })
test('@maple-december-grain', async ({ page }) => { await decemberRoute(page, '/grain', /Harvest actuals:.*30,800/) })
test('@maple-december-contracts', async ({ page }) => { await decemberRoute(page, '/grain/contracts', 'MR-2027-001') })
test('@maple-december-storage', async ({ page }) => { await decemberRoute(page, '/grain/storage', 'Maple North Bin') })
test('@maple-december-inventory-compliance', async ({ page }) => { await decemberRoute(page, '/inventory', 'Maple East 160', 'Compliance') })
test('@maple-december-programs-progress', async ({ page }) => { await decemberRoute(page, '/programs', 'Post-emerge synthetic pass', 'Season progress') })
test('@maple-december-harvest', async ({ page }) => { await decemberRoute(page, '/harvest', /130,900/) })
test('@maple-december-scouting', async ({ page }) => { await decemberRoute(page, '/scouting', 'Synthetic waterhemp at south gate') })
test('@maple-december-tasks', async ({ page }) => { await decemberRoute(page, '/tasks', 'Inspect Maple south gate') })
test('@maple-december-concurrent-preflight remains read-only in two fresh browser contexts', async ({ browser }) => {
