...
declare global { interface Window { __farmRxMayIdentityObservations: string[]; __farmRxMayIdentityConsumptions: { draft: number; operation: number }; __farmRxMayClockObservations: string[] } }
...
async function installDeterminismAndFence(page: Page) {
  ...
  await page.routeWebSocket(/^(?:ws|wss):\/\//, async route => {
    ...
    if (!allowed.has(url.origin)) { forbidden.push(route.url()); await route.close({ code: 1008, reason: 'May proof permits loopback only' }); return }
  ...
  await page.addInitScript(({ fixedMs, draft, operation }) => {
    ...
    Object.defineProperty(crypto, 'randomUUID', { configurable: true, value: () => {
      ...
      if (stack.includes('/src/ProgramsModule.tsx') && consumptions.draft === 0) { consumptions.draft++; observations.push(`draft:${draft}`); return draft }
      if (stack.includes('/src/data/QueuedProgramsRepository.ts') && consumptions.operation === 0) { consumptions.operation++; observations.push(`operation:${operation}`); return operation }
      return original()
    } })
...
test('@may-write marks the Maple pass applied and exposes its draft Inventory record', async ({ page }) => {
  const forbidden = await installDeterminismAndFence(page)
  const unsafeApi: string[] = []
  const productRpcs: string[] = []
  let signedIn = false
  page.on('request', request => { const path = new URL(request.url()).pathname; if (!signedIn) return; if (path.startsWith('/rest/v1/rpc/')) productRpcs.push(path); if (path.startsWith('/rest/v1/') && !['GET', 'HEAD', 'OPTIONS'].includes(request.method())) unsafeApi.push(`${request.method()} ${path}`) })
  await signIn(page)
  signedIn = true
  ...
  expect(productRpcs).toEqual(['/rest/v1/rpc/mark_program_pass_applied'])
  expect(unsafeApi).toEqual(['POST /rest/v1/rpc/mark_program_pass_applied'])
...
test('@may-write-phone phone view confirms the applied pass without writing', async ({ page }) => {
  ...
  const forbidden = await installDeterminismAndFence(page)
  const unsafeApi: string[] = []; const productRpcs: string[] = []; let signedIn = false
  page.on('request', request => { const path = new URL(request.url()).pathname; if (!signedIn) return; if (path.startsWith('/rest/v1/rpc/')) productRpcs.push(path); if (path.startsWith('/rest/v1/') && !['GET', 'HEAD', 'OPTIONS'].includes(request.method())) unsafeApi.push(`${request.method()} ${path}`) })
  await signIn(page); signedIn = true; await page.getByRole('link', { name: 'Programs' }).click(); await page.getByRole('button', { name: 'Season progress' }).click()
  const pass = page.locator('section.tracker-pass').filter({ hasText: 'Post-emerge synthetic pass' })
  ...
  expect(productRpcs).toEqual([])
  expect(unsafeApi).toEqual([])
