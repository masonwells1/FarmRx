...
test('@harvest-ridge-canonical-extension-in adds one independent inbound movement', async ({ page }) => {
  ...
  await form.getByLabel('Bushels').fill('2600'); await form.getByLabel('Commodity').selectOption('corn_yellow'); await form.getByLabel('Crop year').selectOption('2027'); await form.getByLabel('Date').fill('2027-11-06'); await id(page, ids.inbound); await form.getByRole('button', { name: 'Add movement' }).click()
