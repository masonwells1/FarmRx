...
async function installDeterminismAndFence(page: Page, requests: ReturnType<typeof createSeasonRequestClassifier>) {
  ...
  page.on('response', response => {
    if (response.url().startsWith('http://127.0.0.1:55321/') && response.status() >= 400) console.log(`LOCAL_RESPONSE ${response.request().method()} ${new URL(response.url()).pathname} ${response.status()}`)
  })
  page.on('requestfailed', request => {
    if (request.url().startsWith('http://127.0.0.1:55321/')) console.log(`LOCAL_REQUEST_FAILED ${request.method()} ${new URL(request.url()).pathname} ${request.failure()?.errorText ?? 'unknown'}`)
  })
...
test('@june-write records one exact manual Maple application after a read-only Weather check', async ({ page }) => {
  ...
  expect(requests.observedTargetMutationRpcs, 'target mutation RPC ran before the June write action').toEqual([])
...
test('@june-write-phone shows 90 gallons and completed compliance without writing', async ({ page }) => {
  ...
  await page.getByRole('navigation').getByRole('button', { name: 'More' }).click(); await page.getByRole('link', { name: 'Inventory' }).click(); await page.getByRole('button', { name: 'On-hand shelf' }).click(); await expect(page.getByText('90 gal', { exact: true })).toBeVisible()
