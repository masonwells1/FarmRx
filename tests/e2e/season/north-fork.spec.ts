...
test('@north-fork-nf2 owner explicitly enables rep sharing once', async ({ context, page }) => {
  ...
  await sharing.click()
  const sharingConfirmation = page.getByRole('dialog')
  await expect(sharingConfirmation).toContainText('grain, financial, and Soil Rx data')
  await sharingConfirmation.getByRole('button', { name: 'Turn sharing on' }).click()
...
test('@north-fork-nf7 concurrent owner disables sharing and stale rep loses private state', async ({ browser, context, page }) => {
  ...
  const sharing = owner.page.getByRole('switch', { name: 'Share my grain, financial, and Soil Rx data with my Crop RX rep' })
  await expect(sharing).toHaveAttribute('aria-checked', 'true')
  await sharing.click()
  await expect(sharing).toHaveAttribute('aria-checked', 'false')
